...
MOCK_BASE_CLASS(MockProviderNoSQL, datarepository::IProviderNoSQLInterface){
    MOCK_METHOD(GetDataSet, 1, moja::Vector(const std::string&)) MOCK_METHOD(Count, 0, int())};
...
MOCK_BASE_CLASS(MockProviderNoSQLPocoMongoDB, datarepository::ProviderNoSQLPocoMongoDB){
   ...
   MOCK_METHOD(SendQueryRequest, 3, moja::Vector(const std::string&, const std::string&, int),
                        SendQueryRequest_filter) MOCK_METHOD(Count, 0, int())};
