...
ProviderNoSQLPocoMongoDB::ProviderNoSQLPocoMongoDB(DynamicObject settings) : _connected(false), _cache(10000) {
   ...
   try {
      ...
      auto connection_error_message =
          (boost::format("host %1%,  port=%2%, database=%3%") % _host % _port % _database).str();
...
#endif
      // >> CHECK RESPONSE <<
      if (response.documents().size() > 0) ConvertPocoMongoDocumentToDynamic(result, response.documents());
...
DynamicVector ProviderNoSQLPocoMongoDB::SendQueryRequest(const std::string& query, const std::string& fields,
                                                         int numberToReturn) const {
   try {
      DynamicVector result;
