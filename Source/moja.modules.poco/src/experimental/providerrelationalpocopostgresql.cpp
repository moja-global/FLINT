...
ProviderRelationalPocoPostgreSQL::ProviderRelationalPocoPostgreSQL(DynamicObject settings) : _cache(10000) {
...
Dynamic ProviderRelationalPocoPostgreSQL::GetDataSet(const std::string& query) const {
   ...
   switch (resultcode) {
         ...
         if (PQntuples(pResult) != 0) {
            for (int j = 0; j < nCols; j++) {
               ...
               if (columnType != 16 && columnType != 18 && columnType != 19 && columnType != 20 && columnType != 21 &&
                   ...
                   columnType != 700 && columnType != 701 && columnType != 790 && columnType != 1042 &&
                   columnType != 1043) {
               ...
               switch (columnType) {
                  case 16:
                     columnTypes[j] = ColumnType::BOOLEAN;
                     break;
                  case 18:
                     columnTypes[j] = ColumnType::CHARACTER;
                     break;
                  case 19:
                     columnTypes[j] = ColumnType::NAME;
                     break;
                  case 20:
                     columnTypes[j] = ColumnType::INT8OID;
                     break;
                  case 21:
                     columnTypes[j] = ColumnType::INT2OID;
                     break;
                  case 23:
                     columnTypes[j] = ColumnType::INTEGER;
                     break;
                  case 24:
                     columnTypes[j] = ColumnType::REGPROCOID;
                     break;
                  case 25:
                     columnTypes[j] = ColumnType::STRING;
                     break;
                  case 26:
                     columnTypes[j] = ColumnType::OIDOID;
                     break;
                  case 114:
                     columnTypes[j] = ColumnType::JSONOID;
                     break;
                  case 700:
                     columnTypes[j] = ColumnType::FLOAT;
                     break;
                  case 701:
                     columnTypes[j] = ColumnType::DOUBLE;
                     break;
                  case 790:
                     columnTypes[j] = ColumnType::MONEY;
                     break;
                  case 1042:
                     columnTypes[j] = ColumnType::VARCHAR2;
                     break;
                  case 1043:
                     columnTypes[j] = ColumnType::VARCHAR;
                     break;
                  default:
                     PQclear(pResult);
                  ...
                  if (PQgetisnull(pResult, i, k)) {
                     ...
                     switch (columnTypes[k]) {
                        ...
                        case ColumnType::VARCHAR: {
                           auto value =
                               rawValue == nullptr ? Dynamic() : Dynamic(std::string(static_cast<char*>(rawValue)));
                           row.insert(columnNames[k], value);
                           break;
                        }
                        case ColumnType::OIDOID:
                           row.insert(columnNames[k], rawValue);
                           break;
                        case ColumnType::REGPROCOID:
                           row.insert(columnNames[k], rawValue);
                           break;
                        case ColumnType::INT2OID:
                           row.insert(columnNames[k], rawValue);
                           break;
      ...
      case PGRES_SINGLE_TUPLE:
         MOJA_LOG_ERROR << "ProviderRelationalPocoPostgreSQL: "
