...
void PocoMongoDBQueryTransform::configure(DynamicObject config, const flint::ILandUnitController& landUnitController, moja::datarepository::DataRepository& dataRepository) {
	_config = config;
...
std::vector<std::string> PocoMongoDBQueryTransform::extractTokens(const std::string& tokenType, const std::string& query) {
...
void PocoMongoDBQueryTransform::replaceTokens(const std::string& tokenType, std::string& query, std::vector<std::string> values) {
	std::vector<std::string> tokens;
	std::string tokenStart = "{" + tokenType + ":";
	auto tokenEnd = '}';
