...
struct ProviderRelationalPostgresTestsFixture {
   ...
   ProviderRelationalPostgresTestsFixture() {
      connectionstring = "host=" POSTGRESQL_HOST " user=" POSTGRESQL_USER " password=" POSTGRESQL_PWD
                         " dbname=" POSTGRESQL_DB " port=" POSTGRESQL_PORT;
      ...
      auto pResult = PQexec(_conn,
                            "DROP Schema IF EXISTS ProviderRelationalLibpqPostgresTestsFixture_Schema CASCADE; CREATE "
                            "SCHEMA ProviderRelationalLibpqPostgresTestsFixture_Schema; DROP TABLE IF EXISTS "
                            "ProviderRelationalLibpqPostgresTestsFixture_Schema.JunkTable; CREATE TABLE "
                            "ProviderRelationalLibpqPostgresTestsFixture_Schema.JunkTable (id INTEGER);");
...
BOOST_AUTO_TEST_CASE(datarepository_ProviderRelationalLibpqPostgreSQL_libpq_simple_query_with_bin_and_text) {
   ...
   SendQuery(
       "INSERT INTO ProviderRelationalLibpqPostgresTestsFixture_Schema.test1 values(1, 'joe''s place', "
       "'\\000\\001\\002\\003\\004');INSERT INTO ProviderRelationalLibpqPostgresTestsFixture_Schema.test1 values(2, "
       "'ho there', '\\004\\003\\002\\001\\000');");
   ...
   res = PQexecParams(conn, "SELECT * FROM ProviderRelationalLibpqPostgresTestsFixture_Schema.test1 WHERE t = $1",
                      1,                 /* one param */
   ...
   res = PQexecParams(conn, "SELECT * FROM ProviderRelationalLibpqPostgresTestsFixture_Schema.test1 WHERE i = $1::int4",
                      1,                                           /* one param */
                      NULL,                                        /* let the backend deduce param type */
...
BOOST_AUTO_TEST_CASE(datarepository_ProviderRelationalLibpqPostgreSQL_libpq_using_copy_to_insert) {
   ...
   SendQuery(
       "CREATE TABLE ProviderRelationalLibpqPostgresTestsFixture_Schema.test_copy_table (Id INT PRIMARY KEY NOT NULL, "
       "item_01 TEXT,item_02 INT,item_03 CHAR(50),item_04 REAL)");
   //	SendQuery("INSERT INTO ProviderRelationalLibpqPostgresTestsFixture_Schema.test_copy_table VALUES (1,
   //'test',1,'test text', 1.1111)");
   ...
   res = PQexec(conn,
                "COPY ProviderRelationalLibpqPostgresTestsFixture_Schema.test_copy_table FROM STDIN WITH NULL AS '';");
...
BOOST_AUTO_TEST_CASE(
    datarepository_ProviderRelationalLibpqPostgreSQL_ConstructorThrowsExceptionIfTargetDatabaseNotFound) {
   connectionstring = "host=" POSTGRESQL_HOST " user=" POSTGRESQL_USER " password=" POSTGRESQL_PWD
                      " dbname=DoesNotExist"
...
BOOST_AUTO_TEST_CASE(
    datarepository_ProviderRelationalLibpqPostgreSQL_GetDataSetReturnsEmptyTableIfQueryReturnsNoResults) {
   // Setup
   ...
   ProviderRelationalLibpqPostgreSQL provider(settings);
   auto sql =
       "SELECT * FROM ProviderRelationalLibpqPostgresTestsFixture_Schema.Test WHERE Test.data LIKE 'should not be "
       "found'";
...
BOOST_AUTO_TEST_CASE(datarepository_ProviderRelationalLibpqPostgreSQL_SimpleDoubleDataSet) {
   // Setup
   SendQuery(
       "CREATE TABLE ProviderRelationalLibpqPostgresTestsFixture_Schema.SimpleDoubleDataSet (data DOUBLE PRECISION)");
...
BOOST_AUTO_TEST_CASE(datarepository_ProviderRelationalLibpqPostgreSQL_ComplexMixedDataSet) {
   // Setup
   SendQuery(
       "CREATE TABLE ProviderRelationalLibpqPostgresTestsFixture_Schema.ComplexMixedDataSet (col0 VARCHAR, col1 INT, "
       "col2 DOUBLE PRECISION)");
   SendQuery(
       "INSERT INTO ProviderRelationalLibpqPostgresTestsFixture_Schema.ComplexMixedDataSet VALUES ('first', 1, 1.5)");
   SendQuery(
       "INSERT INTO ProviderRelationalLibpqPostgresTestsFixture_Schema.ComplexMixedDataSet VALUES ('second', 2, 2.5)");
