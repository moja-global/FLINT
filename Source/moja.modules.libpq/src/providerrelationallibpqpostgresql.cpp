...
public:
	PostgresConnection(const std::string& connInfo) : _conn{ PQconnectdb(connInfo.c_str()) } {
		...
		if (PQstatus(_conn) != CONNECTION_OK) {
			std::string msg = (boost::format("Database connection failed - '%1%' with connection string '%2%") % PQerrorMessage(_conn) % connInfo).str();
...
public:
	...
	PostgresResult(const PostgresConnection& conn, const std::string& query) {
		_result = PQexec(conn, query.c_str());
	}
...
public:
	...
	DynamicVar GetDataSet(const std::string& query) const {
				...
				for (int i = 0; i < nRows; i++) {
					...
					for (int k = 0; k < nCols; k++) {
						if (pg_result.is_field_null(i, k)) {
							...
							case PostgresResult::ColumnType::CHARACTER:		row.insert(columnNames[k], rawValue);	break;
							case PostgresResult::ColumnType::NAME:			row.insert(columnNames[k], rawValue);	break;
							...
							case PostgresResult::ColumnType::JSONOID:		row.insert(columnNames[k], rawValue);	break;
							...
							case PostgresResult::ColumnType::MONEY:			row.insert(columnNames[k], rawValue);	break;
							case PostgresResult::ColumnType::VARCHAR2:		row.insert(columnNames[k], rawValue);	break;
							...
							case PostgresResult::ColumnType::OIDOID:		row.insert(columnNames[k], rawValue);	break;
							case PostgresResult::ColumnType::REGPROCOID:	row.insert(columnNames[k], rawValue);	break;
		...
		case PGRES_FATAL_ERROR: {
			...
			BOOST_THROW_EXCEPTION(datarepository::QueryException() << datarepository::SQL(query) << datarepository::SQLError(msg));
			break;
		}
		case PGRES_COPY_BOTH:
			break;
		case PGRES_SINGLE_TUPLE:
			MOJA_LOG_ERROR << "ProviderRelationalLibpqPostgreSQL: " << "Not sure what to do with PGRES_SINGLE_TUPLE";
...
ProviderRelationalLibpqPostgreSQL::ProviderRelationalLibpqPostgreSQL(DynamicObject settings) : pimpl{ std::make_unique<impl>(settings) } {}
