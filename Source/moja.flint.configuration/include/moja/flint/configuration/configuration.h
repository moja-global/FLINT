...
class CONFIGURATION_API Configuration {
  public:
   ...
   void setLocalDomain(LocalDomainType type, LocalDomainIterationType iterationType, bool doLogging, int numThreads,
                       ...
                       DynamicObject settings = moja::DynamicObject(), TimeStepping stepping = TimeStepping::Monthly);
   ...
   void addPool(const std::string& name, const std::string& description, const std::string& units, double scale,
                int order, double initValue = 0.0);
   ...
   void addExternalPool(const std::string& name, const std::string& description, const std::string& units, double scale,
                        int order, const std::string& libraryName, const std::string transformTypeName,
...
inline std::vector<const Library*> Configuration::libraries() const { return copy(_libraries); }

inline std::vector<const Provider*> Configuration::providers() const { return copy(_providers); }

inline std::vector<const Pool*> Configuration::pools() const { return copy(_pools); }

inline std::vector<const ExternalPool*> Configuration::externalPools() const { return copy(_externalPools); }

inline std::vector<const Variable*> Configuration::variables() const { return copy(_variables); }
...
inline Spinup* Configuration::spinup() const { return _spinup.get(); }
