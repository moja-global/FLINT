...
public:
	...
	explicit JSON2ConfigurationProvider(const std::vector<std::string>& configFilePath, const std::vector<std::string>& configProviderFilePath);
	virtual ~JSON2ConfigurationProvider() = default;

	virtual std::shared_ptr<Configuration> createConfiguration() override;

	std::vector<std::string> configFilePath() const {
		return _configFilePath;
	}
	std::vector<std::string> configProviderFilePath() const {
		return _configProviderFilePath;
	}
...
private:
    ...
    bool createSpecialPools(const std::string& poolName, const Poco::DynamicStruct& poolSettings, Configuration& config) const;
	bool createSpecialVariables(const std::pair<const std::string, DynamicVar>& keyValPair, Configuration& config, bool isSpinup) const;
