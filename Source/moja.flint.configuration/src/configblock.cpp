...
std::string ConfigBlock::toString() const {
   return (boost::format("Row: %1% Col: %2%") % boost::io::group(std::setfill(' '), std::setw(3), std::abs(_row)) %
           boost::io::group(std::setfill(' '), std::setw(3), std::abs(_col)))
       .str();
}

double ConfigBlock::left() const { return _tile.xOrigin() + _col * _blockSizeX * _tile.xPixelSize(); }

double ConfigBlock::top() const { return _tile.yOrigin() + _tile.ySize() - _row * _blockSizeY * _tile.yPixelSize(); }

double ConfigBlock::xPixelSize() const { return _tile.xPixelSize(); }

double ConfigBlock::yPixelSize() const { return _tile.yPixelSize(); }
