	...
	if (file.is_open()) {
		...
		for (std::string name : names) {
			...
			if (!definedInParsedResult) {
				...
				auto finalDynamicObjContents = finalDynamicObj.isEmpty() ? DynamicObject() : finalDynamicObj.extract<const DynamicObject>();

				auto currentDynamicObj = parsePocoVarToDynamic(currentConfigObj);				
				auto currentObjContents = currentDynamicObj.isEmpty() ? DynamicObject() : currentDynamicObj.extract<const DynamicObject>();
...
void JSON2ConfigurationProvider::createLocalDomain(DynamicVar& parsedJSON, Configuration& config) const {
	...
	if (localDomainType != LocalDomainType::Point) {
		...
		auto iterationType = LocalDomainIterationType::LandscapeTiles; // TODO: confirm this - I've set this as the default for now
		...
		config.setLocalDomain(localDomainType, iterationType, doLogging,
			numThreads, sequencerLibrary, sequencer,
		...
		case LocalDomainIterationType::ASpatialMongoIndex: {
			...
			if (landscape.contains("process_all"))
				asmiIteration->set_processAlldocuments(landscape["process_all"]);
		...
		case LocalDomainIterationType::LandscapeTiles: {
			...
			for (auto& tileConfig : landscape["tiles"].extract<DynamicVector>()) {
				...
				ltIteration->landscapeTiles().push_back(ConfigTile(
					xIndex, yIndex, ltIteration->tileSizeX(), ltIteration->tileSizeY(),
					ltIteration->xPixels(), ltIteration->yPixels()));
			}
			break;
		}
		...
		case LocalDomainIterationType::NotAnIteration: break;
		default: break;
		}
	} else {
		config.setLocalDomain(localDomainType, LocalDomainIterationType::NotAnIteration,
			true, 1, sequencerLibrary, sequencer,
			simulateLandUnit, landUnitBuildSuccess, settings);
	}
	config.localDomain()->operationManagerObject()->set_name(operationManagerSettings["name"].extract<const std::string>());
...
void JSON2ConfigurationProvider::createSpinup(DynamicVar& parsedJSON, Configuration& config) const {
	...
	if (enabled) {
		for (auto param : {
			"sequencer_library", "sequencer", "simulateLandUnit", "landUnitBuildSuccess"
		}) {
			if (!spinupStruct.contains(param)) {
				BOOST_THROW_EXCEPTION(ModuleParamsException()
					<< Param((boost::format("Spinup.%1%") % param).str()));
			}
		}
...
bool JSON2ConfigurationProvider::createSpecialPools(const std::string& poolName, const DynamicStruct& poolSettings, Configuration& config) const {
...
bool JSON2ConfigurationProvider::createSpecialVariables(const std::pair<const std::string, DynamicVar>& keyValPair, Configuration& config, bool isSpinup) const {
	...
	if (keyValPair.second.isStruct()) {
		...
		if (ss.size() == 1) {
			...
			if (keyValPairSS->first == "flintdata") {
				...
				auto& settings = varStruct.contains("settings") ? varStruct["settings"].extract<const DynamicObject>() : DynamicObject();
				isSpinup ? spinup->addFlintDataVariable(varName, libraryName, typeName, settings) : config.addFlintDataVariable(varName, libraryName, typeName, settings);
				return true;
			}
			if (keyValPairSS->first == "transform") {
				...
				auto typeName = varStruct["type"].extract<const std::string>();
				auto libraryName = varStruct["library"].extract<const std::string>();
				isSpinup ? spinup->addExternalVariable(varName, libraryName, typeName, varStruct) : config.addExternalVariable(varName, libraryName, typeName, varStruct);
				return true;
			}
		}
	}
...
void JSON2ConfigurationProvider::createVariables(DynamicVar& parsedJSON, Configuration& config, bool isSpinup) const {
	...
	if (variablesItemVar.isStruct()) {
		...
		for (const auto& keyValPair : variablesItem) {
			...
			if (createSpecialVariables(keyValPair, config, isSpinup))
				continue;
		...
		for (auto& value : arr) {
			...
			if (value.isStruct()) {
				...
				if (value.size() == 1) {
					...
					MOJA_LOG_ERROR << "Variable defined in array (index " << index << ") has has struct with more than one item defined)";
				}
			} 
			else {
				...
				MOJA_LOG_ERROR << "Variable defined in array (index " << index << ") has no value (not Struct or Array): " << varName;
			}
		}
	}
}
