...
void Configuration::Copy(const Configuration& src) {
	setLocalDomain(src._localDomain->type(),
		src._localDomain->iterationType(),
		src._localDomain->doLogging(),
		src._localDomain->numThreads(),
		src._localDomain->sequencerLibrary(),
		src._localDomain->sequencer(),
		src._localDomain->simulateLandUnit(),
		src._localDomain->landUnitBuildSuccess(),
		src._localDomain->settings(),
		src._localDomain->timeStepping()
		);
	...
	if (src._localDomain->type() != LocalDomainType::Point) {
		_localDomain->setLandscapeObject(src._localDomain->landscapeObject()->providerName(), src._localDomain->iterationType());
		...
		if (src.localDomain()->iterationType() == LocalDomainIterationType::ASpatialMongoIndex) {
			...
			for (auto val : iterationSrc->documentIds())
				iteration->addDocumentId(val);
		}
			...
			for (auto& val : iterationSrc->landscapeTiles())
				iteration->addConfigTile(val);
		}
			...
			for (auto& val : iterationSrc->tileIndexList())
				iteration->addTileIndex(val);
		}
			...
			for (auto& val : iterationSrc->blockIndexList())
				iteration->addBlockIndex(val);
		}
			...
			for (auto& val : iterationSrc->cellIndexList())
				iteration->addCellIndex(val);
		}
	}
...
void Configuration::setLocalDomain(
	...
	_localDomain = std::make_shared<LocalDomain>(type, iterationType, doLogging, numThreads, sequencerLibrary, sequencer, simulateLandUnit, landUnitBuildSuccess, settings, stepping);
}

void Configuration::addLibrary(const std::string& path, const std::string& library, const std::string& name, LibraryType type) {
...
void Configuration::addProvider(const std::string& name, const std::string& library, const std::string& providerType, moja::DynamicObject settings) {
...
void Configuration::addPool(const std::string& name, const std::string& description, const std::string& units, double scale, int order, double initValue) {
...
void Configuration::addExternalVariable(const std::string& name, const std::string& libraryName, const std::string transformTypeName, DynamicObject settings) {
...
void Configuration::addExternalPool(const std::string& name, const std::string& description, const std::string& units, double scale, int order,
...
void Configuration::addFlintDataVariable(const std::string& name, const std::string& libraryName, const std::string flintDataTypeName, DynamicObject settings) {
...
void Configuration::addModule(const std::string& libraryName, const std::string& name, int order, bool isProxy, DynamicObject settings) {
	// It is an error for two modules to have the same order - simulation
	// results could be inconsistent.
	auto sameOrder = std::find_if(_modules.begin(),
		_modules.end(),
		[order](std::shared_ptr<Module> other) {
		return other->order() == order;
	});

	if (sameOrder != _modules.end()) {
		throw ModuleOrderOverlapException()
			<< Order(order)
			<< ModuleNames({ libraryName, name, (*sameOrder)->name() });
	}

	auto module = std::make_shared<Module>(libraryName, name, order, isProxy, settings);
	_modules.push_back(module);
	std::sort(_modules.begin(),
		_modules.end(),
		[](std::shared_ptr<Module> lhs, std::shared_ptr<Module> rhs) {
		return lhs->order() < rhs->order();
	});
}

void Configuration::setSpinup(
	bool enabled, 
	const std::string& sequencerLibrary, 
	const std::string& sequencerName,
	const std::string& simulateLandUnit, 
	const std::string& landUnitBuildSuccess,
	DynamicObject settings) {
	_spinup = std::make_shared<Spinup>(enabled, sequencerLibrary, sequencerName, simulateLandUnit, landUnitBuildSuccess, settings);
}
