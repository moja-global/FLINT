...
namespace configuration {
	...
	void Spinup::addExternalVariable(const std::string& name,
		const std::string& libraryName,
	...
	void Spinup::addFlintDataVariable(const std::string& name, const std::string& libraryName, const std::string flintDataTypeName, DynamicObject settings) {
	...
	void Spinup::addSpinupModule(const std::string& libraryName, const std::string& name, int order, bool createNew, DynamicObject settings) {
		...
		auto sameOrder = std::find_if(_modules.begin(),
			_modules.end(),
			[order](std::shared_ptr<SpinupModule> other) {
		...
		if (sameOrder != _modules.end()) {
			throw SpinupModuleOrderOverlapException()
				<< Order(order)
				<< SpinupModuleNames({ libraryName, name, (*sameOrder)->name() });
		}
