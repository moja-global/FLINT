...
namespace configuration {
	...
	double ConfigCell::lon() const {
		return _block.left() + _block.xPixelSize() * _col;
	}

	double ConfigCell::lat() const {
		return _block.top() - _block.yPixelSize() * (_row + 1);
	}
	...
	std::string ConfigCell::toString() const {
		return (boost::format("%1%,%2% (%3%,%4%)")
			% _row
			% _col
			% boost::io::group(std::setprecision(15), lon())
