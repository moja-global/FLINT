...
BOOST_AUTO_TEST_CASE(flint_configuration_Configuration_ConstructorThrowsExceptionIfEndDateIsNotAfterStartDate) {
	...
	auto badEndDates = {
		DateTime(y, mo, d, h, mn, s, ms, us),
		DateTime(y - 1, mo, d, h, mn, s, ms, us),
		DateTime(y, mo - 1, d, h, mn, s, ms, us),
		DateTime(y, mo, d - 1, h, mn, s, ms, us),
		DateTime(y, mo, d, h - 1, mn, s, ms, us),
		DateTime(y, mo, d, h, mn - 1, s, ms, us),
		DateTime(y, mo, d, h, mn, s - 1, ms, us),
		DateTime(y, mo, d, h, mn, s, ms - 1, us),
...
BOOST_AUTO_TEST_CASE(flint_configuration_Configuration_AddProviderThrowsExceptionIfNameIsEmpty) {
	...
	auto badNames = { "", "  " };
	for (auto name : badNames) {
		BOOST_CHECK_THROW(config.addProvider(name, name, name, settings), moja::flint::configuration::ProviderInvalidNameTypeException);
}
}
...
BOOST_AUTO_TEST_CASE(flint_configuration_Configuration_SetLocalDomainThrowsExceptionIfSequencerIsEmpty) {
	auto badNames = { "", "  " };
	for (auto name : badNames) {
		BOOST_CHECK_THROW(config.setLocalDomain(LocalDomainType::SpatiallyReferencedSQL, LocalDomainIterationType::LandscapeTiles, true, 1, "sequencer_library", name, "dummy", "dummy", DynamicObject()), std::invalid_argument);
	}
}
...
BOOST_AUTO_TEST_CASE(flint_configuration_Configuration_SetLocalDomainThrowsExceptionIfSimulateLandUnitIsEmpty) {
	auto badNames = { "", "  " };
	for (auto name : badNames) {
		BOOST_CHECK_THROW(config.setLocalDomain(LocalDomainType::SpatiallyReferencedSQL, LocalDomainIterationType::LandscapeTiles, true, 1, "sequencer_library", "dummy", name, "dummy", DynamicObject()), std::invalid_argument);
	}
}
...
BOOST_AUTO_TEST_CASE(flint_configuration_Configuration_SetLocalDomain) {
	...
	config.setLocalDomain(type, ittype, true, 1, sequencerLibrary, sequencer, simulateLandUnit, landUnitBuildSuccess, settings);
