...
AspatialTileInfo::AspatialTileInfo(
	const AspatialTileInfoCollection& tileInfoCollection,
	int id) : _tileInfoCollection(&tileInfoCollection) {

	if (id < 1) {
		BOOST_THROW_EXCEPTION(LandscapeDefinitionException()
            << Component("id")
			<< Constraint("> 0"));
	}
...
AspatialTileInfoIterator AspatialTileInfo::begin() {
	return AspatialTileInfoIterator(*this);
}

AspatialTileInfoIterator AspatialTileInfo::end() {
	return AspatialTileInfoIterator(*this, false);
}
