...
std::map<std::string, std::string> ProviderManager::FindProviderPathsInDirectory(const std::string& directoryName,
   ...
   while (dirIterator != end) {
      ...
      if (icompare(file, 0, prefix.size(), prefix) == 0 &&
         ...
         // std::cout << "[FindProviderPathsInDirectory 3]" << NamePattern << " ** " << providerName << " ** " <<
         // providerPath.toString() << " ** " << match << std::endl;
...
IProviderInterface::Ptr ProviderManager::GetProvider(const std::string& providerName) {
   ...
   if (providerInfoIt == std::end(_providers)) return nullptr;

   std::shared_ptr<ProviderInfoBase> providerInfo = (*providerInfoIt).second;
   if (providerInfo->InitializeProvider == nullptr) throw LibraryLoadException("Failed to initialise the provider");
   ...
   if (providerInfo->InitializeProvider == nullptr) {
      ...
      if (ProviderFileToLoad.exists()) {
         if (CheckProviderCompatibility(ProviderFileToLoad.path())) {
            ...
            externalProviderInfo->InitializeProvider =
                (InitializeProviderFunctionPtr)externalProviderInfo->Handle->getSymbol("InitializeProvider");
            externalProviderInfo->DestroyProvider =
                (DestroyProviderFunctionPtr)externalProviderInfo->Handle->getSymbol("DestroyProvider");
         } else {
