...
std::shared_ptr<IProviderInterface> DataRepository::createProvider(const std::string& name, const std::string& library, const std::string& providerType, const DynamicObject& settings) const {
...
void DataRepository::addProvider(const std::string& name, const std::string& providerType, std::shared_ptr<IProviderInterface> provider) {
	const auto& p = _providers.find(name);
	if (p == _providers.end()) {
		_providers.insert(std::pair<std::string, std::shared_ptr<IProviderInterface>>(name, provider));
	}
	else {
		BOOST_THROW_EXCEPTION(moja::datarepository::ProviderAlreadyExistsException() << ProviderName(name) << ProviderType(providerType));
	}
}
