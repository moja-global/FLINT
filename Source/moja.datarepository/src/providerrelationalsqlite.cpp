...
public:
	...
	SQLiteStatement(const SQLiteConnection& conn, const std::string& query) {
		if (sqlite3_prepare_v2(conn, query.c_str(), int(query.size()), &_stmt, nullptr) != SQLITE_OK) {
			BOOST_THROW_EXCEPTION(QueryException() << SQL(query) << SQLError(
				sqlite3_errmsg(conn)));
		}
	}
	...
	ColumnType column_type(int col) const {
		...
		if (columnType != SQLITE_INTEGER &&
			...
			if (columnTypeStr == COLUMN_TEXT ||
				columnTypeStr == COLUMN_VARCHAR) {
				columnType = SQLITE_TEXT;
			}
...
public:
	impl(DynamicObject settings) :
		_path(settings["path"].convert<std::string>()),
		_conn(_path),
		_cache(10000) {}
	...
	DynamicVar GetDataSet(const std::string& query) const {
		...
		for (; resultCode == SQLITE_ROW || resultCode == SQLITE_BUSY || resultCode == SQLITE_LOCKED || resultCode == SQLITE_IOERR; resultCode = stmt.step()) {
			DynamicObject row;
...
DynamicVar ProviderRelationalSQLite::GetDataSet(const std::string& query) const {
