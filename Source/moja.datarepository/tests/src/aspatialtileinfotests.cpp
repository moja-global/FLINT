...
BOOST_AUTO_TEST_CASE(datarepository_ConstructorThrowsExceptionIfIdIsLessThanOne) {
   ...
   auto badIds = {0, -1, -100};
   for (auto id : badIds) {
      BOOST_CHECK_THROW(AspatialTileInfo(mockCollection, id), LandscapeDefinitionException);
   }
}
...
BOOST_AUTO_TEST_CASE(datarepository_LandUnitAtIndexReturnsExpectedLandUnit) {
   ...
   std::shared_ptr<std::map<Int64, DynamicVar>> luData = std::make_shared<std::map<Int64, DynamicVar>>();
