...
public:
	...
	private:
		...
		unsigned _tileTop;		unsigned _blockTop;		unsigned _cellTop;
		unsigned _tileLeft;		unsigned _blockLeft;	unsigned _cellLeft;
		unsigned _tileRight;	unsigned _blockRight;	unsigned _cellRight;
		unsigned _tileBottom;	unsigned _blockBottom;	unsigned _cellBottom;
	};

	typedef _iterator<false> iterator;
	typedef _iterator<true> const_iterator;

	explicit SpatialCellsCollection(const TileBlockCellIndexer& indexer) :_indexer(indexer), _tileIdx(0, indexer), _blockIdx(0, 0, indexer), _hasAreaOfInterest(false), _hasTileIdx(false), _hasBlockIdx(false) {}
	SpatialCellsCollection(Rectangle& areaOfInterest, const TileBlockCellIndexer& indexer) :_indexer(indexer), _tileIdx(0, indexer), _blockIdx(0, 0, indexer), _areaOfInterest(areaOfInterest), _hasAreaOfInterest(true), _hasTileIdx(false), _hasBlockIdx(false) {}
	SpatialCellsCollection(const TileIdx& tileIdx, const TileBlockCellIndexer& indexer) :_indexer(indexer), _tileIdx(tileIdx), _blockIdx(0, 0, indexer), _hasAreaOfInterest(false), _hasTileIdx(true), _hasBlockIdx(false) {}
	SpatialCellsCollection(const BlockIdx& blockIdx, const TileBlockCellIndexer& indexer) :_indexer(indexer), _tileIdx(0, indexer), _blockIdx(blockIdx), _hasAreaOfInterest(false), _hasTileIdx(false), _hasBlockIdx(true) {}
...
inline SpatialCellsCollection::iterator SpatialCellsCollection::begin() {
	if (_hasAreaOfInterest)
		return iterator(_areaOfInterest, _indexer);
	if (_hasTileIdx)
		return iterator(_tileIdx, _indexer);
	if (_hasBlockIdx)
		return iterator(_blockIdx, _indexer);
	return iterator(_indexer);
}

inline SpatialCellsCollection::iterator SpatialCellsCollection::end() {
	return iterator(_indexer, true);
}

inline SpatialCellsCollection::const_iterator SpatialCellsCollection::begin() const {
	if (_hasAreaOfInterest)
		return const_iterator(_areaOfInterest, _indexer);
	if (_hasTileIdx)
		return const_iterator(_tileIdx, _indexer);
	if (_hasBlockIdx)
		return const_iterator(_blockIdx, _indexer);
...
template <bool is_const_iterator>
SpatialCellsCollection::_iterator<is_const_iterator>::_iterator(const Rectangle& areaOfInterest, const TileBlockCellIndexer& indexer) :
...
template <bool is_const_iterator>
SpatialCellsCollection::_iterator<is_const_iterator>::_iterator(const TileIdx& tileIdx, const TileBlockCellIndexer& indexer) :
...
template <bool is_const_iterator>
SpatialCellsCollection::_iterator<is_const_iterator>::_iterator(const BlockIdx& blockIdx, const TileBlockCellIndexer& indexer) :
...
template <bool is_const_iterator>
typename SpatialCellsCollection::_iterator<is_const_iterator>::self_type& SpatialCellsCollection::_iterator<is_const_iterator>::operator++() {
	increment();
	return *this;
}

template <bool is_const_iterator>
typename SpatialCellsCollection::_iterator<is_const_iterator>::self_type SpatialCellsCollection::_iterator<is_const_iterator>::operator++(int) {
...
template <bool is_const_iterator>
typename SpatialCellsCollection::_iterator<is_const_iterator>::ValueReferenceType SpatialCellsCollection::_iterator<is_const_iterator>::operator*() { return _cell; }

template <bool is_const_iterator>
typename SpatialCellsCollection::_iterator<is_const_iterator>::ValuePointerType SpatialCellsCollection::_iterator<is_const_iterator>::operator->() { return &_cell; }

template <bool is_const_iterator>
bool SpatialCellsCollection::_iterator<is_const_iterator>::operator==(const self_type& rhs) {
	return (_cell.cellIdx == rhs._cell.cellIdx
		&& _cell.blockIdx == rhs._cell.blockIdx
		&& _cell.tileIdx == rhs._cell.tileIdx);
}

template <bool is_const_iterator>
bool SpatialCellsCollection::_iterator<is_const_iterator>::operator!=(const self_type& rhs) { return !(*this == rhs); }
...
void SpatialCellsCollection::_iterator<is_const_iterator>::increment() {
	...
	if (!_hasAreaOfInterest && !_hasTileIdx && !_hasBlockIdx)
		return;
	...
	while (!foundNext) {
		...
		if ((tileX == _tileLeft		&& blockX < _blockLeft)
			|| (tileY == _tileTop		&& blockY < _blockTop)
			|| (tileX == _tileRight		&& blockX > _blockRight)
			|| (tileY == _tileBottom	&& blockY > _blockBottom)
			) {
