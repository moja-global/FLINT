...
public:
	...
	/** The provider object for this provider.  We actually *own* this provider, so it's lifetime is controlled by the scope of this shared pointer. */
...
private:
	...
	std::map<std::string, std::string> FindProviderPathsInDirectory(const std::string& InDirectoryName, const std::string&  NamePattern, bool useSuffix) const;
