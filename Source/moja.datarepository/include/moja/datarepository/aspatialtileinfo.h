...
class DATAREPOSITORY_API AspatialTileInfoIterator
	: public std::iterator<std::forward_iterator_tag, LandUnitInfo> {
public:
	AspatialTileInfoIterator() { }

	AspatialTileInfoIterator(AspatialTileInfo& tileInfo, bool start = true)
		: _tileInfo(&tileInfo) {
	...
	bool operator==(const AspatialTileInfoIterator& other) const {
		return _currentIndex == other._currentIndex
			&& _tileInfo->id() == other._tileInfo->id();
	}
