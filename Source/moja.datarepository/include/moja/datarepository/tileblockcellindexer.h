...
//// --------------------------------------------------------------------------------------------
//
//struct DATAREPOSITORY_API Point {
//	double lat;	// y
//	double lon;	// x
//};
//
//struct DATAREPOSITORY_API XYIndex {
//	UInt32 X;
//	UInt32 Y;
//};
//
//struct DATAREPOSITORY_API Size {
//	double height;
//	double width;
//};
//
...
public:
	...
	double bottom() const;		// y-coordinate that is the sum of the Y and Height property values of this Rectangle structure
...
public:
	TileBlockCellIndexer(double tileLatSize, double tileLonSize, double blockLatSize, double blockLonSize, double cellLatSize, double cellLonSize);
	TileBlockCellIndexer(UInt32 tileRows, UInt32 tileCols, UInt32 blockRows, UInt32 blockCols, UInt32 cellRows, UInt32 cellCols);

	void SetGlobalData();

	void SanityChecksLatLon(double tileLatSize, double tileLonSize, double blockLatSize, double blockLonSize, double cellLatSize, double cellLonSize);
	void SanityChecksIndex(UInt32 tileRows, UInt32 tileCols, UInt32 blockRows, UInt32 blockCols, UInt32 cellRows, UInt32 cellCols);
	...
	bool compare(const TileBlockCellIndexer& indexer) const { return (cellResCompareValueLon == indexer.cellResCompareValueLon && cellResCompareValueLat == indexer.cellResCompareValueLat); }
	bool compare(TileBlockCellIndexer& indexer) const { return (cellResCompareValueLon == indexer.cellResCompareValueLon && cellResCompareValueLat == indexer.cellResCompareValueLat); }
...
inline UInt32 TileBlockCellIndexer::convOtoI(double o, double partSize) { return UInt32((o / partSize) + MathEx::k0Plus); }
...
// Then Tile  Lat size & Lon size (user defined tile  size that fits in the global size a whole number of times), i.e. 1.0
// Then Block Lat size & Lon size (user defined block size that fits in the tile   size a whole number of times), i.e. 0.1
// Then Cell  Lat size & Lon size (user defined cell  size that fits in the block  size a whole number of times), i.e. 0.00025
struct DATAREPOSITORY_API TileIdx {
	TileIdx() = delete;

	TileIdx(const Point point, const TileBlockCellIndexer& indexer) :
...
struct DATAREPOSITORY_API BlockIdx {
	...
	BlockIdx(const BlockIdx& src) :
		tileCols(src.tileCols),
		tileIdx(src.tileIdx),
		blockCols(src.blockCols),
		blockIdx(src.blockIdx) {}
	...
	BlockIdx(const XYIndex tile, const XYIndex block, const TileBlockCellIndexer& indexer) : 
		...
		if (t_idx > indexer.tileDesc.indexLimit - 1) {
			...
			BOOST_THROW_EXCEPTION(TileBlockCellIndexerInvalidParameterException() << Component(ss.str()) << Constraint("tileindex > indexLimit"));
		}

		if (b_idx > indexer.blockDesc.indexLimit - 1) {
			std::stringstream ss;
			ss << "block.X:" << block.X << "block.Y:" << block.Y;
			BOOST_THROW_EXCEPTION(TileBlockCellIndexerInvalidParameterException() << Component(ss.str()) << Constraint("blockindex > indexLimit"));
		}
...
inline size_t CellIdx::hash() const {
	return hash::hash_combine(tileIdx, blockIdx, cellIdx);
}
...
class CellIdxHasher {
public:
	size_t operator()(const CellIdx& ci) const {
		return hash::hash_combine(ci.tileIdx, ci.blockIdx, ci.cellIdx);
	}
};
...
struct DATAREPOSITORY_API blockStats {
	...
	blockStats() : 
		cellProcessing(0),  cells(0), processed(0), errors(0) {
		tileIdx = -1;
		blockIdx = -1;
	}
...
inline void blockStats::endCell() {
	cellProcessing += cell_clock::now() - cellStart;
}
...
struct DATAREPOSITORY_API blockCollectionStats {
	...
	blockCollectionStats() : curTileIdx(-1), curBlockIdx(-1), curBlock(nullptr), 
							 blocksProcessed(0), cellsInspected(0), cellsProcessed(0) {}
	...
	bool checkEndBlock(const T& idx) {
		if ((curTileIdx != -1 && curBlockIdx != -1)
			&& (curTileIdx != idx.tileIdx || curBlockIdx != idx.blockIdx)) {
			endCurBlock();
			return true;
		}
		return false;
...
inline void blockCollectionStats::startProcessedCell() const {
	curBlock->startCell();
}
inline void blockCollectionStats::endProcessedCell() const {
	curBlock->endCell();
}
inline void blockCollectionStats::addInspectedCell() const {
	curBlock->cells++;
}
...
struct DATAREPOSITORY_API tileStats {
	...
	tileStats()
		: startTime(clock::now()), endTime(clock::now()), tileIdx(-1) {}
...
struct DATAREPOSITORY_API tileCollectionStats {
	...
	tileCollectionStats() : startTime(clock::now()), endTime(clock::now()), tilesProcessed(0), totalTileTime(0), curTileIdx(-1), curTile(nullptr) {}
