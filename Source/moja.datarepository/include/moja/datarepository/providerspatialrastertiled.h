...
class DATAREPOSITORY_API ProviderSpatialRasterTiled final : public IProviderSpatialRasterInterface {
  public:
   explicit ProviderSpatialRasterTiled(std::shared_ptr<RasterReaderFactoryInterface> rasterReaderFactory,
   ...
   SpatialCellsCollection cells(Rectangle areaOfInterest) const override {
      return SpatialCellsCollection(areaOfInterest, *(_indexer.get()));
   };
   SpatialCellsCollection cells(const TileIdx& tileIdx) const override {
      return SpatialCellsCollection(tileIdx, *(_indexer.get()));
   };
   SpatialCellsCollection cells(const BlockIdx& blockIdx) const override {
      return SpatialCellsCollection(blockIdx, *(_indexer.get()));
   };

   SpatialBlocksCollection blocks() const override { return SpatialBlocksCollection(*(_indexer.get())); };
   SpatialBlocksCollection blocks(Rectangle areaOfInterest) const override {
      return SpatialBlocksCollection(areaOfInterest, *(_indexer.get()));
   };
   SpatialBlocksCollection blocks(const TileIdx& tileIdx) const override {
      return SpatialBlocksCollection(tileIdx, *(_indexer.get()));
   };

   SpatialTilesCollection tiles() const override { return SpatialTilesCollection(*(_indexer.get())); };
   SpatialTilesCollection tiles(Rectangle areaOfInterest) const override {
      return SpatialTilesCollection(areaOfInterest, *(_indexer.get()));
   };
...
inline const TileBlockCellIndexer& ProviderSpatialRasterTiled::indexer() { return *(_indexer.get()); }
