...
BOOST_AUTO_TEST_CASE(core_timespan_TotalHours) {
    ...
    BOOST_CHECK_EQUAL(ts.totalHours(),
          ts.days() * 24
        + ts.hours());
}
...
BOOST_AUTO_TEST_CASE(core_timespan_TotalMinutes) {
	...
	BOOST_CHECK(std::abs(ts.totalMinutes() - 
		( ts.days() * 24 * 60 
		+ ts.hours() * 60 
		+ ts.minutes())) < 0.000000001);
}

BOOST_AUTO_TEST_CASE(core_timespan_TotalSeconds) {
    moja::Timespan ts(1, 1, 1, 1, 0);

	BOOST_CHECK(std::abs(ts.totalSeconds() -
        ( ts.days() * 24 * 60 * 60
        + ts.hours() * 60 * 60
        + ts.minutes() * 60
...
BOOST_AUTO_TEST_CASE(core_timespan_TotalMilliseconds) {
	...
	BOOST_CHECK(std::abs(ts.totalMilliseconds() -
        ( ts.days() * 24 * 60 * 60 * 1000
        + ts.hours() * 60 * 60 * 1000
        + ts.minutes() * 60 * 1000
        + ts.seconds() * 1000
        + ts.milliseconds())) < 0.000000001);
}

BOOST_AUTO_TEST_CASE(core_timespan_totalMicroseconds) {
    moja::Timespan ts(0, 0, 0, 1, 1);

	BOOST_CHECK(std::abs(ts.totalMicroseconds() -
        ( ts.days() * 24 * 60 * 60 * 1000 * 1000
        + ts.hours() * 60 * 60 * 1000 * 1000
        + ts.minutes() * 60 * 1000 * 1000
        + ts.seconds() * 1000 * 1000
        + ts.milliseconds() * 1000
        + ts.microseconds())) < 0.000000001);
}
