...
public:
     ...
     */
    void connect(std::function<void(Args...)> const& slot) const {
...
public:
    ...
    bool operator==(Property<T> const& rhs) const {
        return Property<T>::value() == rhs.value();
    }
...
template<> inline Property<double>::Property()
    : _connection(nullptr), _value(0.0) {}

template<> inline Property<float>::Property()
    : _connection(nullptr), _value(0.f) {}

template<> inline Property<short>::Property()
    : _connection(nullptr), _value(0) {}
...
template<> inline Property<bool>::Property()
    : _connection(nullptr), _value(false) {}
...
public:
    ...
    std::type_index type() const override { 
        return typeid(R(Params...)); 
    }

    virtual R invoke(Params... args) const override {
        return (_callee.*_func)(args...);
    }

    R operator()(Params... args) const override {
        return (_callee.*_func)(args...);
    }

    bool operator==(const Delegate& other) const {
        return (&_callee == &other._callee) && (_func == other._func);
    }
...
template<typename ... Args>
void DispatcherObject::callListeners(const Listeners& listeners, short originalSignal, const Listeners& postListeners, Args... args) {
    using Func = void(typename std::remove_reference<Args>::type...);
    std::type_index index(typeid(Func));
