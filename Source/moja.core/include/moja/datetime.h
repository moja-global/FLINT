...
public:
    ...
    DateTime() :
		_date(),
		_tod(std::chrono::hours{ 0 }, std::chrono::minutes{ 0 }, std::chrono::seconds{ 0 }, 
	...
	DateTime(short year, unsigned month, unsigned day, int hour, int minute, int second,
		int millisecond, int microsecond = 0) :
		_date(date::year{ year }, date::month{ month }, date::day{ day }),
		_tod(std::chrono::hours{ hour }, std::chrono::minutes{ minute }, std::chrono::seconds{ second },
    ...
    */
	...
	int millisecond() const { return int(std::chrono::duration_cast<std::chrono::milliseconds>(_tod.subseconds()).count()); }
	...
	DateTime& assign(short year, unsigned month, unsigned day, int hour, int minute, int second, int millisecond, int microsecond);
...
inline bool DateTime::isLeapYear(int y) MOJA_NOEXCEPT {
	return  y % 4 == 0 && (y % 100 != 0 || y % 400 == 0);
}
inline short DateTime::daysInYear(int year) {
...
inline DateTime& DateTime::assign(short year, unsigned month, unsigned day, int hour, int minute, int second, int millisecond, int microsecond) {
	using namespace date;
	_date = year_month_day(date::year{ year }, date::month{ month }, date::day{ day });
	_tod = make_time(std::chrono::hours{ hour }+std::chrono::minutes{ minute }+std::chrono::seconds{ second }+std::chrono::milliseconds{ millisecond }+std::chrono::microseconds{ microsecond });
	return *this;
}
...
inline bool DateTime::operator == (const DateTime& dateTime) const { return _date == dateTime._date && _tod.to_duration() == dateTime._tod.to_duration(); }
...
inline bool DateTime::operator <  (const DateTime& dateTime) const { 
	return _date <  dateTime._date ? true
		: (_date >  dateTime._date ? false
		: (_tod.to_duration() <  dateTime._tod.to_duration())); 
}
