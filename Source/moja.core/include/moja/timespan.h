...
inline int Timespan::hours() const { return std::chrono::duration_cast<std::chrono::hours>(_span).count() % 24; }
...
inline int Timespan::minutes() const { return std::chrono::duration_cast<std::chrono::minutes>(_span).count() % 60; }
...
inline long long Timespan::microseconds() const { return _span.count() % 1000000; }
...
inline bool Timespan::operator==(const Timespan& ts) const { return _span == ts._span; }
...
inline bool Timespan::operator==(const std::chrono::microseconds& ts) const { return _span == ts; }
...
inline void swap(Timespan& s1, Timespan& s2) MOJA_NOEXCEPT { s1.swap(s2); }
