...
Timespan::Timespan(std::chrono::microseconds microSeconds) : _span(microSeconds) {}

Timespan::Timespan(long seconds, long microSeconds)
    : _span(std::chrono::seconds{seconds} + std::chrono::microseconds{microSeconds}) {}

Timespan::Timespan(int days, int hours, int minutes, int seconds, int microSeconds)
    : _span(std::chrono::microseconds{microSeconds} + std::chrono::seconds{seconds} + std::chrono::minutes{minutes} +
            std::chrono::hours{hours} + date::days{days}) {}

Timespan::Timespan(const Timespan& timespan) : _span(timespan._span) {}

int Timespan::days() const { return std::chrono::duration_cast<date::days>(_span).count(); }
...
void Timespan::swap(Timespan& timespan) MOJA_NOEXCEPT { std::swap(_span, timespan._span); }
...
Timespan Timespan::operator+(const std::chrono::microseconds& d) const { return Timespan(_span + d); }
