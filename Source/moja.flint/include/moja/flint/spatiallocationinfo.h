...
public:
	...
	SpatialLocationInfo(
		UInt32 tileIdx, UInt32 blockIdx, UInt32 cellIdx, 
		Point tileLatLon, Point blockLatLon, Point cellLatLon, 
		...
		double landUnitArea) : 
			...
			_tileLatLon(tileLatLon), _blockLatLon(blockLatLon), _cellLatLon(cellLatLon), 
			_tileRows(tileRows), _tileCols(tileCols), 
			_blockRows(blockRows), _blockCols(blockCols), 
			_cellRows(cellRows), _cellCols(cellCols),
			_landUnitArea(landUnitArea), _errorCode(0) {}
	...
	virtual void configure(DynamicObject config, const flint::ILandUnitController& landUnitController, moja::datarepository::DataRepository& dataRepository) override {
		if (config.contains("tileIdx"))			{ _tileIdx			= config["tileIdx"]; }		else _tileIdx			= 0;
		if (config.contains("blockIdx"))		{ _blockIdx			= config["blockIdx"]; }		else _blockIdx			= 0;
		if (config.contains("cellIdx"))			{ _cellIdx			= config["cellIdx"]; }		else _cellIdx			= 0;

		if (config.contains("latTile"))			{ _tileLatLon.lat	= config["latTile"]; }		else _tileLatLon.lat	= 0.0;
		if (config.contains("lonTile"))			{ _tileLatLon.lon	= config["lonTile"]; }		else _tileLatLon.lon	= 0.0;
		if (config.contains("latBlock"))		{ _blockLatLon.lat	= config["latBlock"]; }		else _blockLatLon.lat	= 0.0;
		if (config.contains("lonBlock"))		{ _blockLatLon.lon	= config["lonBlock"]; }		else _blockLatLon.lon	= 0.0;
		if (config.contains("latCell"))			{ _cellLatLon.lat	= config["latCell"]; }		else _cellLatLon.lat	= 0.0;
		if (config.contains("lonCell"))			{ _cellLatLon.lon	= config["lonCell"]; }		else _cellLatLon.lon	= 0.0;

		if (config.contains("rowsTile"))		{ _tileRows			= config["tileRows"]; }		else _tileRows			= 0;
		if (config.contains("colsTile"))		{ _tileCols			= config["tileCols"]; }		else _tileCols			= 0;
		if (config.contains("rowsBlock"))		{ _blockRows		= config["BlockRows"]; }	else _blockRows			= 0;
		if (config.contains("colsBlock"))		{ _blockCols		= config["BlockCols"]; }	else _blockCols			= 0;
		if (config.contains("rowsCell"))		{ _cellRows			= config["CellRows"]; }		else _cellRows			= 0;
		if (config.contains("colsCell"))		{ _cellCols			= config["CellCols"]; }		else _cellCols			= 0;

		if (config.contains("landUnitArea"))	{ _landUnitArea		= config["landUnitArea"]; }	else _landUnitArea		= 0.0;

		if (config.contains("randomSeedGlobal"	))	{ _randomSeedGlobal		= config["randomSeedGlobal"	]; }	else _randomSeedGlobal	= 0;
		if (config.contains("randomSeedTile"	))	{ _randomSeedTile		= config["randomSeedTile"	]; }	else _randomSeedTile	= 0;
		if (config.contains("randomSeedBlock"	))	{ _randomSeedBlock		= config["randomSeedBlock"	]; }	else _randomSeedBlock	= 0;
		if (config.contains("randomSeedCell"	))	{ _randomSeedCell		= config["randomSeedCell"	]; }	else _randomSeedCell	= 0;
	}
