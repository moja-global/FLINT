...
public:
	explicit OperationResultFluxIteratorUblas(OperationTransferType transferType, const ModuleMetaData* metaData, moja_ublas_matrix& flux, bool atEnd = false)
	...
	bool isEqual(IOperationResultFluxIterator& it) const override {
		...
		if (_outerIt == _flux.end1() && p->_outerIt == _flux.end1())
			return true;
