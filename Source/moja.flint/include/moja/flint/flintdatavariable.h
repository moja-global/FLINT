...
public:
	FlintDataVariable(std::shared_ptr<IFlintData> flintData, std::string libraryName, std::string variableName, VariableInfo info)
		: _libraryName(libraryName), _variableName(variableName), _info(info), _value(flintData), _flintdata(flintData) {}
	...
	std::shared_ptr<const IFlintData> flintdata() const	{
		return _flintdata;
	}

	std::shared_ptr<IFlintData> flintdata()	{
		return _flintdata;
	}
