...
public:
	typedef std::shared_ptr<IFlintData>(*FactoryFunc)(const std::string&, int, const std::string&, const DynamicObject&);

	virtual ~IFlintData() = default;

	virtual void registerEventFactoryFunc(FactoryFunc func) {}
	virtual void registerEventFactoryFunc(std::function<std::shared_ptr<IFlintData>(const std::string&, int, const std::string&, const DynamicObject&)> func) {}

	virtual void configure(DynamicObject config, const ILandUnitController& landUnitController, datarepository::DataRepository& dataRepository) = 0;
