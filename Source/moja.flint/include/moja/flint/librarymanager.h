...
public:
	...
	void AddLibrary(LibraryType type,
	...
	ProviderInterfacePtr	GetProvider(const std::string& libraryName, const std::string& moduleName, const DynamicObject& settings);

	bool LoadInternalLibrary(std::shared_ptr<FlintLibraryHandles> libraryHandles);

	bool LoadManagedLibrary();

	bool LoadExternalLibrary(const std::string& libraryName,
		const std::string& path = std::string(),
		const std::string& fileName = std::string());
...
private:
	...
	*/
	...
	std::map<std::string, std::string> FindLibraryPathsInDirectory(
