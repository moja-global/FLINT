...
#include "spatiallocationinfo.h"
#include "spinuplandunitcontroller.h"
#include <moja/flint/record.h>
#include "recordaccumulatorwithmutex.h"
...
//	   tileIdx, blockIdx
typedef  Poco::Tuple<Int64, std::string, Int64, Int64, Int64, Int64, Int64, Int64, Int64, Int64, Poco::Nullable<UInt32>, Poco::Nullable<UInt32>> StatsUnitRow;
...
public:
	...
	void mojaLog(const std::string& levelStr, int localDomainId, datarepository::TileIdx* tileIdxObject, datarepository::BlockIdx* blockIdxObject, bool inSeconds, bool showRecordNumber, int recordPosition, int recordCount);

	//static void sqliteCreateTable(bool dropExisting, int localDomainId, std::string databaseName, std::string tableName);
	//static void sqliteWrite(std::shared_ptr<RecordAccumulatorWithMutex<StatsUnitRow>>& collection, int localDomainId, std::string databaseName, std::string tableName);
...
public:
	...
	SpatialTiledLocalDomainController(std::shared_ptr<FlintLibraryHandles> libraryHandles, bool threadedSystem, bool isMaster, bool isThread);
...
private:
	...
	std::map<std::pair<UInt32, UInt32>, std::vector<datarepository::CellIdx>> _blockCellIdxList;	// list of cells to sim, mapped by blockIdx. Used in thread/non-threaded processing
	...
	bool runCell(std::shared_ptr<StatsUnitRecord>& blockStatsUnit, std::shared_ptr<StatsUnitRecord>& blockStatsSpinUpUnit, const datarepository::CellIdx& cell);
