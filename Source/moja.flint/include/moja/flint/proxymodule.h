...
public:
	...
	void addModule(const std::string& libraryName, const std::string& moduleName, std::shared_ptr<IModule> module, const DynamicObject& config) override {
	...
	std::vector<std::tuple<std::string, std::string, std::string>> listModules() const override {
		...
		for (auto item : _modulesData) {
			result.emplace_back(item._libraryName, item._moduleName, item._useVariable ? item._variableName : "varibale not used");
		}
		return result;
	}
