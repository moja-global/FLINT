...
public:
	OutputerStream() : ModuleBase(), _outputToScreen(false), _outputAnnually(false), _outputInfoHeader(false), _outputOnOutputStep(true), _outputOnTimingEndStep(false), _outputOnPostDisturbanceEvent(false) {}
...
protected:
	...
	std::vector<std::tuple<std::string, std::string, std::string, IVariable*>> _variables; // including sub property if it has one
