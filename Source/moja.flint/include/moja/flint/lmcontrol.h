...
public:
	...
	void lmMinimize(int m_dat, int n_par, double[] par, LmEvaluate evaluate, LmPrintOut printout, LMControl control, double[] xValue, double[] yValue) {
		/*** allocate work space. ***/
	...
	void lm_lmdif(int m, int n, double* x, double* fvec, double ftol,
		...
		double* diag, int mode, double factor, LMMinInfo info, int& nfev,
		double* fjac, int* ipvt, double* qtf, double* wa1,
		double* wa2, double* wa3, double* wa4,
		LmEvaluate evaluate, LmPrintOut printout,
		...
		if (info < LMMinInfo::InproperInput)
			return;
	...
	void lm_qrfac(int m, int n, double* a, bool pivot, int* ipvt,
