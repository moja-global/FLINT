...
enum class OperationTransferHandlingType {
	Aggregated = 0,
	Disaggregated = 1
};
...
inline std::string OperationTransferHandlingTypeToString(OperationTransferHandlingType t) {
	switch (t) {
	case OperationTransferHandlingType::Aggregated:		return "Aggregated";
	case OperationTransferHandlingType::Disaggregated:	return "Disaggregated";
	default: break;
	}
...
public:
	...
	IOperation(const ModuleMetaData* metaData, DynamicVar& dataPackage) : _metaData(metaData), _dataPackage(dataPackage), _hasDataPackage(true) {}
	virtual ~IOperation() = default;

	// Some issues to consider with an operation
	//	1/	Transfers can result in a zero move - 0.0 proportional move, pool value is zero on a proportional move, 0 stock move, etc
	//		Should these be ignored or recorded as zero fluxes?
	//	2/	If an operation has no transfers or only zero result transfers should sumbit create an OperationResult or not?
	//	3/	When we have a zero transfer on submite -> commpute should a flux of zero be generated?
	//		it seems Ublas will remove the zero result fluxes while eigen will keep them
	//	4/	Operation transfers can be aggregated or not. The simple implementation keeps them while Matrix versions aggregate
	//
