...
public:
	explicit WriteSystemConfig(Poco::Mutex& fileHandlingMutex) 
		:	ModuleBase(), 
			...
			_writeFrequency(0), _writeOutstepFrequency(0), _writeCount(0), _writeCellProcessed(0), _writeThisCell(false) {}
...
private:
	...
	bool _onNotificationArray[5];	// when to capture the configuration (which notification method) - TimingInit [default], TimingShutdown, Error
	...
	static int convertNotificationStringToIndex(std::string str) {
		if (str == "PreTimingSequence")
			return static_cast<int>(OnNotificationType::PreTimingSequence);
		if (str == "TimingInit")
			return static_cast<int>(OnNotificationType::TimingInit);
		if (str == "TimingShutdown")
			return static_cast<int>(OnNotificationType::TimingShutdown);
		if (str == "OutputStep")
			return static_cast<int>(OnNotificationType::OutputStep);
		if (str == "Error")
			return static_cast<int>(OnNotificationType::Error);
	...
	static std::string convertNotificationToString(OnNotificationType notification) {
		switch (notification) {
		case OnNotificationType::PreTimingSequence	: return "PreTimingSequence";
		case OnNotificationType::TimingInit			: return "TimingInit";
		case OnNotificationType::TimingShutdown		: return "TimingShutdown";
		case OnNotificationType::OutputStep			: return "OutputStep";
		case OnNotificationType::Error				: return "Error";
		}
		return "TimingInit";
	}
};
