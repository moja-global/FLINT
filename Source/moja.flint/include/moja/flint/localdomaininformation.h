...
public:
	...
	void configure(DynamicObject config, const flint::ILandUnitController& landUnitController, moja::datarepository::DataRepository& dataRepository) override {
		...
		if (config.contains("controllerName"))		{ controllerName	= config["controllerName"].extract<const std::string>(); }
		if (config.contains("controllerVersion"))	{ controllerVersion = config["controllerVersion"].extract<const std::string>(); }
		...
		if (config.contains("isMaster"))			{ isMaster			= config["isMaster"]; }
		if (config.contains("isThread"))			{ isThread			= config["isThread"]; }
	}
