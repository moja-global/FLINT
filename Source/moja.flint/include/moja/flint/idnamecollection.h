...
class FLINT_API IdNameCollection : public flint::IFlintData {
  public:
   ...
   void configure(DynamicObject config, const flint::ILandUnitController& landUnitController,
      ...
      _collectionName =
          config.contains("collection_name") ? config["collection_name"].extract<const std::string>() : "unnamed";
      _data.clear();
      _dataLoaded = false;
      ...
      } else if (_landUnitController != nullptr && _dataRepository != nullptr && config.contains("provider") &&
                 config.contains("queryString")) {
   ...
   DynamicObject exportObject() const override {
      ...
      for (auto& dataItem : _data) {
         items.push_back(DynamicObject({{"id", dataItem._id}, {"name", dataItem._name}}));
      }
      object["items"] = items;
      return object;
   };
