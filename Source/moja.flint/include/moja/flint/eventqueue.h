...
class FLINT_API EventBase : public IFlintData {
  ...
  protected:
   EventBase(int id, const std::string& type, const std::string& name) : id(id), type(type), name(name) {}
};
...
class FLINT_API EventQueue : public IFlintData {
  public:
   ...
   void registerEventFactoryFunc(FactoryFunc func) override { _eventFactoryMethods.push_back(func); }

   void registerEventFactoryFunc(
...
inline EventQueue::iterator EventQueue::begin() MOJA_NOEXCEPT { return _queue.begin(); }

inline EventQueue::const_iterator EventQueue::begin() const MOJA_NOEXCEPT { return _queue.begin(); }

inline EventQueue::iterator EventQueue::end() MOJA_NOEXCEPT { return _queue.end(); }

inline EventQueue::const_iterator EventQueue::end() const MOJA_NOEXCEPT { return _queue.end(); }

inline void EventQueue::clear() { _queue.clear(); }

inline EventQueue::size_type EventQueue::size() const MOJA_NOEXCEPT { return _queue.size(); }
...
inline EventQueue::iterator EventQueue::erase(const_iterator pos) { return _queue.erase(pos); }
