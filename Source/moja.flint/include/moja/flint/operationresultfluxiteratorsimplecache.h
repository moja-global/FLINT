...
class OperationResultFluxIteratorSimpleCache : public IOperationResultFluxIterator {
  public:
   explicit OperationResultFluxIteratorSimpleCache(std::vector<OperationResultFluxSimpleCache>& results,
                                                   bool atEnd = false)
       : _results(results) {
   ...
   virtual std::shared_ptr<IOperationResultFluxIterator> createIterator(bool atEnd = false) override {
      return std::make_shared<OperationResultFluxIteratorSimpleCache>(_results, atEnd);
   }
...
#ifdef USE_INT_ITERATOR
      _currentIndex++;
      if (_currentIndex >= _results.size()) _currentIndex = -1;
#else
      ++_it;
#endif
   }
