...
// --------------------------------------------------------------------------------------------
// Purpose of this land unit controller is to do the same as the LandUnitController put to use the same _poolVector of another controller
// Given on construction. Not sure this is the best approach. We could for example use 2 normal LUC and copy the pool values across when required
// An example of how CBM might implement it's Spinup procedure
// Another solution could be to write a Special LUC for Spunup which handles both the spinup and normal run, providing extra methods for the 
// spinUp parts (modules etc).
