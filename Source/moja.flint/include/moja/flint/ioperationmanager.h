...
inline std::string OperationTransferTypeToString(OperationTransferType t) {
	switch (t) {
	case OperationTransferType::Stock: return "Stock";
	case OperationTransferType::Proportional: return "Proportional";
	}
	return "unknown";
}
...
public:
    ...
    virtual const IPool* addPool(const std::string& name, const std::string& description, const std::string& units, double scale, int order, const std::shared_ptr<ITransform> transform) = 0;
