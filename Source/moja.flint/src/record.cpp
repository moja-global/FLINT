...
// -- Date2Record
Date2Record::Date2Record(int year) : _year(year) {}
...
size_t PoolInfoRecord::hash() const {
   if (_hash == -1) _hash = moja::hash::hash_combine(_name);
   return _hash;
}
...
FluxRecord::FluxRecord(bool doAggregation, int localDomainId, Int64 dateId, Poco::Nullable<Int64> moduleInfoId,
                       Int64 itemCount, int srcPoolInfoId, int sinkPoolInfoId, double flux)
    : _doAggregation(doAggregation),
      _localDomainId(localDomainId),
      _dateId(dateId),
      _moduleInfoId(moduleInfoId),
      _itemCount(itemCount),
      _srcPoolInfoId(srcPoolInfoId),
      _sinkPoolInfoId(sinkPoolInfoId),
      _flux(flux) {}

bool FluxRecord::operator==(const Record<FluxRow>& other) const {
   if (!_doAggregation) return false;

   auto otherRow = other.asPersistable();
   return _localDomainId == otherRow.get<1>() && _dateId == otherRow.get<2>() && _moduleInfoId == otherRow.get<3>() &&
          _srcPoolInfoId == otherRow.get<5>() && _sinkPoolInfoId == otherRow.get<6>();
}
