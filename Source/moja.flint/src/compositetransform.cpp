...
namespace flint {
    ...
    void CompositeTransform::configure(
        ...
        if (varNames.size() < 1) {
            BOOST_THROW_EXCEPTION(IncompleteConfigurationException()
                << Item("vars")
                << Details("At least one variable name required"));
        }
        ...
        for (std::string varName : varNames) {
            if (std::find(_varNames.begin(), _varNames.end(), varName) != _varNames.end()) {
                BOOST_THROW_EXCEPTION(IncompleteConfigurationException()
                    << Item("vars")
                    << Details("Duplicate variable reference"));
            }
    ...
    const DynamicVar& CompositeTransform::value() const {
        if (_variables.empty()) {
            for (auto varName : _varNames) {
                ...
                if (var == nullptr) {
                    BOOST_THROW_EXCEPTION(VariableNotFoundException()
