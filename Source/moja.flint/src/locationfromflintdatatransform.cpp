...
void LocationFromFlintDataTransform::configure(DynamicObject config, const ILandUnitController& landUnitController, datarepository::DataRepository& dataRepository) {
	_landUnitController = &landUnitController;
	_dataRepository = &dataRepository;
	_provider = std::static_pointer_cast<datarepository::IProviderSpatialRasterInterface>(_dataRepository->getProvider(config["provider"].convert<std::string>()));
...
const DynamicVar& LocationFromFlintDataTransform::value() const {
	...
	_cachedValue = _provider->GetValue(_dataId, _spatialLocationInfo->_cellLatLon.lat, _spatialLocationInfo->_cellLatLon.lon);
	return _cachedValue;
}
