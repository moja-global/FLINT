...
void OutputerStream::configure(const DynamicObject& config) {
	_fileName						= config.contains("output_filename")					? config["output_filename"].extract<std::string>() : "OutputerStreamFlux.txt";
	...
	_outputOnTimingEndStep			= config.contains("output_on_timingendstep")			? bool(config["output_on_timingendstep"]) : false;
	_outputOnPostDisturbanceEvent	= config.contains("output_on_postdisturbanceevent")		? bool(config["output_on_postdisturbanceevent"]) : false;
...
void OutputerStream::outputHeader(std::ostream& stream) const {
	...
	stream << "notification" << DL_CHR << "step" << DL_CHR << "stepDate" << DL_CHR << "fracOfStep" DL_CHR << "stepLenInYears" << DL_CHR;
...
void OutputerStream::outputInit(std::ostream& stream) {
	...
	stream << "onTimingPostInit" << DL_CHR << timingL.step() << DL_CHR << timingL.curEndDate().addMicroseconds(-1) << DL_CHR << timingL.fractionOfStep() << DL_CHR << timingL.stepLengthInYears() << DL_CHR;
...
void OutputerStream::outputEndStep(const std::string& notification, std::ostream& stream) {
	const auto& timingL = *_landUnitData->timing();
    stream << notification << DL_CHR << timingL.step() << DL_CHR << timingL.curEndDate().addMicroseconds(-1) << DL_CHR << timingL.fractionOfStep() << DL_CHR << timingL.stepLengthInYears() << DL_CHR;
	stream << std::setprecision(STOCK_PRECISION);
	auto pools = _landUnitData->poolCollection();
	for (auto& it : pools) {
		stream << it->value() << DL_CHR;
...
void OutputerStream::onOutputStep() {
	if (_outputOnOutputStep) {
		...
		if (   !_outputAnnually 
			|| timingL.curStartDate().month() == 12)
			outputEndStep("onOutputStep", _output);
	}
}
...
void OutputerStream::onTimingEndStep() {
	if (_outputOnTimingEndStep)
		outputEndStep("onTimingEndStep", _output);
}
...
void OutputerStream::onPostDisturbanceEvent() {
	if (_outputOnPostDisturbanceEvent)
		outputEndStep("onPostDisturbanceEvent", _output);
}
