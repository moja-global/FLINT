...
void DebugModuleWithAnythingInItEvent::configure(DynamicObject config, const flint::ILandUnitController& landUnitController, moja::datarepository::DataRepository& dataRepository) {
...
void EventQueue::configure(DynamicObject config, const flint::ILandUnitController& landUnitController, datarepository::DataRepository& dataRepository) {
	_queue.clear();
			...
			if (!_eventFactoryMethodsNew.empty()) {
				for (auto& func : _eventFactoryMethodsNew) {
					...
					if (eventObject) {
						...
						_queue.emplace_back(EventQueueItem{ eventDate, std::static_pointer_cast<flint::EventBase>(eventObject) });
						break;
					}
				}
			}
