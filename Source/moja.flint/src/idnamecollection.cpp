...
IdNameCollection::DataRecord& IdNameCollection::getRecord(int index) {
   if (_useDataBuilder && !_dataLoaded) {
      auto provider = std::static_pointer_cast<datarepository::IProviderRelationalInterface>(
          _dataRepository->getProvider(_providerName));
      ...
      if (!dataSet.isEmpty()) {
         ...
         for (auto& item : arr) {
            int id = item["id"];
            if (first && id != 0) _data.emplace_back(DataRecord{0, "Unknown"});
            first = false;
