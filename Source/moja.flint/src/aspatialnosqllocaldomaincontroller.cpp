...
void AspatialNoSQLLocalDomainController::configure(const configuration::Configuration& config) {
	...
	_provider = std::static_pointer_cast<moja::datarepository::IProviderNoSQLInterface>(_dataRepository.getProvider(landscapeObject->providerName()));
	...
	//_tiles = std::make_unique<moja::datarepository::AspatialTileInfoCollection>(*provider.get(), iterator->maxTileSize(), iterator->tileCacheSize());
}
...
void AspatialNoSQLLocalDomainController::run() {
	...
	for (auto lu : _idSet) {
		...
		try {
			MOJA_LOG_INFO  << std::setfill(' ') << std::setw(10) << current << " of " << std::setfill(' ') << std::setw(10) << _count;
