...
void WriteSystemConfig::configure(const DynamicObject& config) {
	...
	_writeOutstepFrequency	= config.contains("write_outstep_frequency") ? config["write_outstep_frequency"].convert<UInt32>() : 0;

	// set all notifications to false
	for (auto& val : _onNotificationArray)
		val = false;
	if (config.contains("on_notification")) {
		...
		if (dynamic.isVector()) {
			...
			_onNotificationArray[convertNotificationStringToIndex(config["on_notification"].convert<std::string>())] = true;
		}
	} else {
...
void WriteSystemConfig::onOutputStep() {
	...
	if ((_writeOutstepFrequency != 0) && ((timestep - 1) % _writeOutstepFrequency) != 0)
		return;
...
void WriteSystemConfig::onError(std::string msg) {
	if (_writeThisCell && _onNotificationArray[static_cast<int>(OnNotificationType::Error)])
		WriteConfig("Error");
}
...
#define OUTPUT_LEVEL_WS(level)		for (int i = 0; i < level; i++) fout << "\t";
...
void outputDynamicToStream(std::ofstream& fout, const DynamicVar& object, int level) {
	if (object.isStruct()) {
		...
		if (object.type() == typeid(DateTime)) {
			...
			std::string simpleDateStr = (boost::format("{ \"$date\": \"%1%/%2%/%3%\" }") % dt.year() % dt.month() % dt.day()).str();
			fout << simpleDateStr;
...
void WriteSystemConfig::WriteConfig(std::string notificationStr) const {
	...
	Poco::File workingFolder(_outputPath);
	auto configFilename = (boost::format("%1%%2%%3%%4%%5%_%6%_%7%_%8%_%9%_%10%.json")
		% workingFolder.path()
		% Poco::Path::separator()
		% _name
		% Poco::Path::separator()
	...
	if (configFile.exists())
		configFile.remove(false);	// delete existing config file
	...
	for (auto& library : config->libraries()) {
		if (library->hasPath()) {
			libraries[library->path()] = DynamicObject(
			{
	...
	localdomain["start_date_init"]		= (boost::format("%1%/%2%/%3%") % config->startDate().year() % config->startDate().month() % config->startDate().day()).str();
	localdomain["start_date"]			= (boost::format("%1%/%2%/%3%") % timing->curStartDate().year() % timing->curStartDate().month() % timing->curStartDate().day()).str();
	localdomain["end_date"]				= (boost::format("%1%/%2%/%3%") % config->endDate().year() % config->endDate().month() % config->endDate().day()).str();
	...
	switch (localDomainConfig->type()) {
		case configuration::LocalDomainType::SpatialTiled:
		{
			switch (localDomainConfig->landscapeObject()->iterationType()) {
				case configuration::LocalDomainIterationType::NotAnIteration:		break;
				case configuration::LocalDomainIterationType::ASpatialIndex:		break;
				case configuration::LocalDomainIterationType::ASpatialMongoIndex:	break;
				case configuration::LocalDomainIterationType::AreaOfInterest:		break;
				case configuration::LocalDomainIterationType::LandscapeTiles:		break;
				...
				case configuration::LocalDomainIterationType::CellIndex: {
					localdomain["landscape"] = DynamicObject({
						//{ "iteration_type",		configuration::convertLocalDomainIterationTypeToStr(localDomainConfig->landscapeObject()->iterationType()) },
						{ "iteration_type",		"CellIndex" },
						{ "num_threads",		localDomainConfig->numThreads() },
						{ "provider",			localDomainConfig->landscapeObject()->providerName() },
						{ "cells",				DynamicVector({
						DynamicObject({
							{ "tile_index",		_spatialLocationInfo->_tileIdx },
	...
	for (auto& variable : _landUnitData->variables()) {
		...
		if (variable->isFlintData()) {
			...
			auto flintDataVar = DynamicObject(
				{
					{ "flintdata", DynamicObject( {
						{ "library",	ptr->libraryName() },
	...
	for (auto& module : config->modules()) {
		modules[module->name()] = DynamicObject( 
			{
	...
	auto configOutput =	DynamicObject(
		{ 
