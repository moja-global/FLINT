...
// --------------------------------------------------------------------------------------------
StockOperationSimple::StockOperationSimple(OperationManagerSimple* manager, double timeScale, const ModuleMetaData* metaData)
	: IOperation(metaData), _manager(manager), _timeScale(timeScale) {
	_transfers.reserve(20);
}

StockOperationSimple::StockOperationSimple(OperationManagerSimple* manager, double timeScale, const ModuleMetaData* metaData, DynamicVar& dataPackage) 
...
StockOperationSimple* StockOperationSimple::addTransfer(const IPool* source, const IPool* sink, double value) {
	//MOJA_LOG_DEBUG << "Simple Transfer (stck) - src: " << source->idx() << ", snk: " << sink->idx() << ", value:" << value;
...
OperationTransferType StockOperationSimple::transferType() const {
	return OperationTransferType::Stock;
}

// --------------------------------------------------------------------------------------------

void StockOperationSimple::submitOperation() {
