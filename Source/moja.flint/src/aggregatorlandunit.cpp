...
void AggregatorLandUnit::subscribe(NotificationCenter& notificationCenter) {
	...
	notificationCenter.subscribe(signals::LocalDomainProcessingUnitInit,		&AggregatorLandUnit::onLocalDomainProcessingUnitInit,		*this);
	notificationCenter.subscribe(signals::LocalDomainProcessingUnitShutdown,	&AggregatorLandUnit::onLocalDomainProcessingUnitShutdown,	*this);
...
void AggregatorLandUnit::onSystemInit() {
	...
	try {
		...
		for (auto& pool : _landUnitData->poolCollection()) {
			auto poolInfoRecord = std::make_shared<PoolInfoRecord>(pool->name(), pool->description(), pool->idx(), pool->order(), pool->scale(), pool->units());
...
void AggregatorLandUnit::onSystemShutdown() {
	_simulationUnitData->_endSystemTime = DateTime::now();
}
...
void AggregatorLandUnit::onLocalDomainShutdown() {
	_simulationUnitData->_endLocalDomainTime = DateTime::now();
}
...
void AggregatorLandUnit::onTimingShutdown() {
	...
	for (auto& flux : _fluxesLU) {
		...
		auto fluxRecord = std::make_shared<FluxRecord>(
			_aggregatorLandUnitSharedData._aggregateSinkAndSource, 
			_simulationUnitData->_localDomainId,
			flux._dateRecordId, 
			flux._moduleId, 
			1, 
			flux._srcIx, 
...
void AggregatorLandUnit::recordFluxSet() {
	...
	if (_aggregatorLandUnitSharedData._outputMonth12Only) {
		...
		auto dateRecord = std::make_shared<DateRecord>(curStep, curSubStep, reportingEndDate.year(), reportingEndDate.month(), reportingEndDate.day(), timing->fractionOfStep(), timing->stepLengthInYears());
	...
	for (auto operationResult : _landUnitData->getOperationLastAppliedIterator()) {
		...
		if (_aggregatorLandUnitSharedData._moduleInfoOn && metaData != nullptr) {
			auto moduleRecord = std::make_shared<ModuleInfoRecord>(metaData->libraryType, metaData->libraryInfoId, metaData->moduleType, metaData->moduleId, metaData->moduleName);
