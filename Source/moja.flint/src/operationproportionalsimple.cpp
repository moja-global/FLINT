...
ProportionalOperationSimple::ProportionalOperationSimple(OperationManagerSimple* manager, const std::vector<double>& pools, double timeScale, const ModuleMetaData* metaData)
	: IOperation(metaData), _manager(manager), _timeScale(timeScale), _pools(pools) {
	_transfers.reserve(20);
}

ProportionalOperationSimple::ProportionalOperationSimple(OperationManagerSimple* manager, const std::vector<double>& pools, double timeScale, const ModuleMetaData* metaData, DynamicVar& dataPackage)
...
ProportionalOperationSimple* ProportionalOperationSimple::addTransfer(const IPool* source, const IPool* sink, double value) {
	//MOJA_LOG_DEBUG << "Simple Transfer (prop) - src: " << source->idx() << ", snk: " << sink->idx() << ", value:" << value;
...
std::shared_ptr<IOperationResult> ProportionalOperationSimple::computeOperation(ITiming& _timing)
	...
	//MOJA_LOG_DEBUG << "Simple computeOperation (prop) - pools: " << boost::join(_pools | boost::adaptors::transformed(static_cast<std::string(*)(double)>(std::to_string)), ", ");
...
OperationTransferType ProportionalOperationSimple::transferType() const {
	return OperationTransferType::Proportional;
}

// --------------------------------------------------------------------------------------------
void ProportionalOperationSimple::submitOperation() {
