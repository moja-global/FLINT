...
void OutputerStreamFlux::outputHeader(std::ostream& stream) const {
	...
	stream << "step" << DL_CHR << "step date" << DL_CHR << "module name" << DL_CHR << "disturbance_type" << DL_CHR << "source pool" DL_CHR << "sink pool" << DL_CHR << "value" << std::endl;
}
...
void OutputerStreamFlux::outputInit(std::ostream& stream) const {
	...
	for (auto operationResult : _landUnitData->getOperationLastAppliedIterator()) {
		...
		for (auto f : operationResult->operationResultFluxCollection()) {
			...
			stream << srcPool->name() << DL_CHR << dstPool->name() << DL_CHR << std::setprecision(STOCK_PRECISION) << val << std::endl;
		}
	}
}
...
void OutputerStreamFlux::onTimingPostInit() {
	outputInit(_output);
}

// --------------------------------------------------------------------------------------------

void OutputerStreamFlux::onTimingEndStep() {
