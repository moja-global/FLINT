...
AspatialLocalDomainThread::AspatialLocalDomainThread(
    ThreadedAspatialLocalDomainController* parent,
    int threadId,
    Poco::Mutex& tileListMutex,
    std::queue<datarepository::AspatialTileInfo>& tileList,
    const configuration::Configuration* config)
        : _parent(parent), _threadConfig(config->startDate(), config->endDate()),
          _threadId(threadId), _tileListMutex(tileListMutex), _tileList(tileList),
          _config(config) {
...
void ThreadedAspatialLocalDomainController::configure(const configuration::Configuration& config) {
   ...
   auto provider = std::static_pointer_cast<moja::datarepository::IProviderRelationalInterface>(
    ...
    for (auto t = 0; t < _numThreads; t++) {
        auto task = std::make_shared<AspatialLocalDomainThread>(
            this, t + 100, _tileListMutex, _tileList, _config);
