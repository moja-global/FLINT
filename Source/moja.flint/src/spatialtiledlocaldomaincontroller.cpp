...
// remaining"; 			std::this_thread::sleep_for(RETRY_SLEEP); 			retry = maxRetries-- > 0;
// if (!retry) { 				MOJA_LOG_DEBUG << localDomainId << ":Exceeded MAX RETIRES (" <<
...
// std::vector<RunSummaryDataRecord>& runSummaryData) { 	auto retry = false; 	auto maxRetries = RETRY_ATTEMPTS;
// do { 		try {
//			retry = false;
//			SQLite::Connector::registerConnector();
...
//			}
//			if (insertRunList)
//				session << "CREATE TABLE IF NOT EXISTS run_list (id INTEGER PRIMARY KEY AUTOINCREMENT,
//run_id VARCHAR(64) NOT NULL, run_desc VARCHAR(255))", now; 			session << "CREATE TABLE IF NOT EXISTS
// run_properties (id INTEGER PRIMARY KEY AUTOINCREMENT, run_id_fk VARCHAR(64), property_name VARCHAR(255), property_info
// VARCHAR(255), property_value VARCHAR(255))", now;
//
//			if (insertRunList) {
//				auto insertStr = (boost::format("INSERT INTO run_list (run_id, run_desc)
//VALUES('%1%','%2%')") % _runId % _runDesc).str(); 				session.begin();
//session << insertStr, now; 				session.commit();
//			}
//
//			session.begin();
//			session << "INSERT INTO run_properties (run_id_fk, property_name, property_info, property_value)
// VALUES(?, ?, ?, ?)", bind(runSummaryData), now; 			session.commit();
// SQLite::Connector::unregisterConnector();
//		}
//		catch (SQLite::DBLockedException&) {
//			MOJA_LOG_DEBUG << localDomainId() << ":DBLockedException - " << maxRetries << " retries
...
void SpatialTiledLocalDomainController::run() {
   ...
   if (_doLogging) {
      if (!_threadedSystem || (_threadedSystem && _isMaster)) {
         ...
         //	StatsUnitRecord tileStatsRecord(i.get<1>(), seconds(i.get<2>()), seconds(i.get<3>()),
         // seconds(i.get<4>()), seconds(i.get<5>()), i.get<6>(), i.get<7>(), i.get<8>(), i.get<9>(), i.get<10>(),
         // i.get<11>(), StatsDurationType::Seconds); 	totalUnits += tileStatsRecord._unitsTotal;
         // totalUnitsProcessed += tileStatsRecord._unitsProcessed; 	tileStatsRecord.mojaLog("Global",
         // localDomainId(), nullptr, nullptr, true);
         //}
