...
namespace moja {
	namespace flint {
		...
		StatsUnitRecord::StatsUnitRecord(std::shared_ptr<StatsUnitRecord>& ptr, StatsDurationType durationType)
			: _runId(ptr->_runId), _elapsedTimeTotal(ptr->_elapsedTimeTotal),
			_elapsedTimeFramework(ptr->_elapsedTimeFramework), _elapsedTimeSpinup(ptr->_elapsedTimeSpinup), _elapsedTimeProcessed(ptr->_elapsedTimeProcessed), _unitsTotal(ptr->_unitsTotal),
			_unitsProcessed(ptr->_unitsProcessed), _unitsNotProcessed(ptr->_unitsNotProcessed), _unitsWithError(ptr->_unitsWithError), _tileIdx(ptr->_tileIdx),
			_blockIdx(ptr->_blockIdx), _durationType(durationType) {}

		// -- StatsUnitRecord
		StatsUnitRecord::StatsUnitRecord(std::string runId,
			duration elapsedTimeTotal, duration elapsedTimeFramework, duration elapsedTimeSpinup, duration elapsedTimeProcessed,
			Int64 unitsTotal, Int64 unitsProcessed, Int64 unitsNotProcessed, Int64 unitsWithError,
			Poco::Nullable<UInt32> tileIdx, Poco::Nullable<UInt32> blockIdx, StatsDurationType durationType)
			: _runId(runId),
			_elapsedTimeTotal(elapsedTimeTotal), _elapsedTimeFramework(elapsedTimeFramework), _elapsedTimeSpinup(elapsedTimeSpinup), _elapsedTimeProcessed(elapsedTimeProcessed),
			_unitsTotal(unitsTotal), _unitsProcessed(unitsProcessed), _unitsNotProcessed(unitsNotProcessed), _unitsWithError(unitsWithError),
			_tileIdx(tileIdx), _blockIdx(blockIdx), _durationType(durationType) {}

		bool StatsUnitRecord::operator==(const Record<StatsUnitRow>& other) const {
			auto otherRow = other.asPersistable();
			return    _tileIdx == otherRow.get<10>()
				&& _blockIdx == otherRow.get<11>();
		}

		size_t StatsUnitRecord::hash() const {
			return moja::hash::hash_combine(_tileIdx, _blockIdx);
		}
		...
		StatsUnitRow StatsUnitRecord::asPersistable() const {
			...
			return StatsUnitRow{
				...
				_runId,
				elapsedTimeTotal		,
				elapsedTimeFramework	,
				elapsedTimeSpinup		,
				elapsedTimeProcessed	,
				_unitsTotal,
				_unitsProcessed,
				_unitsNotProcessed,
				_unitsWithError,
		...
		void StatsUnitRecord::mojaLog(const std::string& levelStr, int localDomainId, datarepository::TileIdx* tileIdxObject, datarepository::BlockIdx* blockIdxObject, bool inSeconds = false, bool showRecordNumber = false, int recordPosition = 0, int recordCount = 0) {
			using namespace std::chrono;
			...
			auto timeTotal = inSeconds ? duration_cast<seconds>(_elapsedTimeTotal) : duration_cast<milliseconds>(_elapsedTimeTotal);
			auto timeProcessed = inSeconds ? duration_cast<seconds>(_elapsedTimeProcessed) : duration_cast<milliseconds>(_elapsedTimeProcessed);

			std::string recordInfoStr = "";
			if (showRecordNumber) {
				recordInfoStr = (boost::format(
					"Record [%1% of %2%]: ")
					% boost::io::group(std::setw(5), recordPosition)
			...
			if (tileIdxObject != nullptr) {
				recordInfoStr += (boost::format(
					"Index [%1%]: ")
					% boost::io::group(std::setw(5), tileIdxObject->toString())
					)
					.str();
			}
			if (blockIdxObject != nullptr) {
				recordInfoStr += (boost::format(
					"Index [%1%]: ")
					% boost::io::group(std::setw(5), blockIdxObject->toString())
					)
					.str();
			}
			MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId << ": Stats level [" << levelStr << "]: " // "Global: "
				<< recordInfoStr
				<< "Time" << (inSeconds ? "(sec)" : "(ms)") << " ["
...
#define RETRY_SLEEP std::chrono::milliseconds(200)
		...
		//void StatsUnitRecord::sqliteCreateTable(bool dropExisting, int localDomainId, std::string databaseName, std::string tableName) {
		//	auto retry = false;
		//	auto maxRetries = RETRY_ATTEMPTS;
		//	do {
		//		try {
		//			retry = false;
		...
		//			session << "CREATE TABLE IF NOT EXISTS " << tableName << " (id UNSIGNED BIG INT NOT NULL, runId VARCHAR(255), elapsedTimeTotal UNSIGNED BIG INT NOT NULL, elapsedTimeFramework UNSIGNED BIG INT NOT NULL, elapsedTimeSpinup UNSIGNED BIG INT NOT NULL, elapsedTimeProcessed UNSIGNED BIG INT NOT NULL, unitsTotal UNSIGNED BIG INT NOT NULL, unitsProcessed UNSIGNED BIG INT NOT NULL, unitsNotProcessed UNSIGNED BIG INT NOT NULL, unitsWithErrors UNSIGNED BIG INT NOT NULL, tileIdx INTEGER, blockIdx INTEGER)", now;
		//
		//			SQLite::Connector::unregisterConnector();
		//		}
		//		catch (SQLite::DBLockedException&) {
		//			MOJA_LOG_DEBUG << localDomainId << ":DBLockedException - " << maxRetries << " retries remaining";
		//			std::this_thread::sleep_for(RETRY_SLEEP);
		//			retry = maxRetries-- > 0;
		//			if (!retry) {
		//				MOJA_LOG_DEBUG << localDomainId << ":Exceeded MAX RETIRES (" << RETRY_ATTEMPTS << ")";
		//				throw;
		//			}
		//		}
		//	} while (retry);
		//}
		//
		//void StatsUnitRecord::sqliteWrite(std::shared_ptr<RecordAccumulatorWithMutex<StatsUnitRow>>& collection, int localDomainId, std::string databaseName, std::string tableName) {
		//	auto retry = false;
		//	auto maxRetries = RETRY_ATTEMPTS;
		//	do {
		//		try {
		...
		//				MOJA_LOG_DEBUG << localDomainId << ":SQLite " << tableName << " - inserted " << collection->size() << " records";
		//				session.begin();
		//				session << "INSERT INTO " << tableName << " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", bind(collection->getPersistableCollection()), now;
		//				session.commit();
		//			}
		//
		//			SQLite::Connector::unregisterConnector();
		...
		public:
			InternalThreadBlocks(int threadId,
				std::string runId,
				Poco::Mutex& blockIdxListMutex,
				std::queue<datarepository::BlockIdx>& blockIdxList,
				int blockIdxListSize,
		...
		SpatialTiledLocalDomainController::InternalThreadBlocks::InternalThreadBlocks(
			...
			std::shared_ptr<RecordAccumulatorWithMutex<StatsUnitRow>>& tileStatsDimension,
			std::shared_ptr<RecordAccumulatorWithMutex<StatsUnitRow>>& blockStatsDimension,
			std::shared_ptr<FlintLibraryHandles> internalLIbraryHandles) : _threadId(threadId), _runId(runId),
			_blockIdxListMutex(blockIdxListMutex),
			_blockIdxList(blockIdxList), _blockCellIdxList(blockCellIdxList),
			_config(config), _threadConfig(config->startDate(), config->endDate()) {

			_ldc = std::make_shared<SpatialTiledLocalDomainController>(
				globalStatsDimension, tileStatsDimension, blockStatsDimension,
				blockIdxListSize, internalLIbraryHandles, true, false, true);
			...
			_threadConfig.localDomain()->setLandscapeObject(
				_config->localDomain()->landscapeObject()->providerName(),
				conf::LocalDomainIterationType::BlockIndex);

			auto ldInfoSettings = DynamicObject({
				{ "controllerName",		"SpatialTiledLocalDomainController" },
			...
			_threadConfig.addFlintDataVariable("flint_system_localdomaininfo",
				"internal.flint",
				"LocalDomainInformation",
				ldInfoSettings);
			...
			_ldc->initialiseData();
			MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << _threadId << ":" << "Thread Created";
		}

		// --------------------------------------------------------------------------------------------

		void SpatialTiledLocalDomainController::InternalThreadBlocks::operator()() {
			MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << _threadId << ":" << "Thread Started ";
			...
			_ldc->shutdown();
			MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << _threadId << ":" << "Thread Finished ";
		}
		...
		SpatialTiledLocalDomainController::SpatialTiledLocalDomainController()
			...
			_globalStatsDimension(nullptr), _tileStatsDimension(nullptr), _blockStatsDimension(nullptr),
			_blockIdxListSize(0), _blockIdxListPosition(0),
			_runSpinUp(false), _spinupLandUnitController(_landUnitController), _spinupSimulateLandUnit(nullptr), _spinupLandUnitBuildSuccess(nullptr),
			_threadedSystem(false), _numThreads(0), _isMaster(false), _isThread(false),
			_internalLIbraryHandles(nullptr),
			_doLogging(false), _resetVariablesForEachLU(false), _writeStatsToSQLIte(false), _createStatsTablesSQLIte(true) {}

		// --------------------------------------------------------------------------------------------

		SpatialTiledLocalDomainController::SpatialTiledLocalDomainController(
			bool threadedSystem, bool isMaster, bool isThread)
			: LocalDomainControllerBase(),
			_globalStatsDimension(nullptr), _tileStatsDimension(nullptr), _blockStatsDimension(nullptr),
			_blockIdxListSize(0), _blockIdxListPosition(0),
			_runSpinUp(false), _spinupLandUnitController(_landUnitController), _spinupSimulateLandUnit(nullptr), _spinupLandUnitBuildSuccess(nullptr),
			_threadedSystem(threadedSystem), _numThreads(0), _isMaster(isMaster), _isThread(isThread),
			_internalLIbraryHandles(nullptr),
			_doLogging(false), _resetVariablesForEachLU(false), _writeStatsToSQLIte(false), _createStatsTablesSQLIte(true) {}
		...
		SpatialTiledLocalDomainController::SpatialTiledLocalDomainController(
			...
			_globalStatsDimension(globalStatsDimension), _tileStatsDimension(tileStatsDimension), _blockStatsDimension(blockStatsDimension),
			_blockIdxListSize(blockIdxListSize), _blockIdxListPosition(0),
			_runSpinUp(false), _spinupLandUnitController(_landUnitController), _spinupSimulateLandUnit(nullptr), _spinupLandUnitBuildSuccess(nullptr),
			_threadedSystem(threadedSystem), _numThreads(0), _isMaster(isMaster), _isThread(isThread),
			_doLogging(false), _resetVariablesForEachLU(false), _writeStatsToSQLIte(false), _createStatsTablesSQLIte(true) {
			_internalLIbraryHandles = libraryHandles;
		...
		void SpatialTiledLocalDomainController::configure(const configuration::Configuration& config) {
			...
			_provider = std::static_pointer_cast<data::IProviderSpatialRasterInterface>(_dataRepository.getProvider(config.localDomain()->landscapeObject()->providerName()));
			...
			_samplingInterval = localDomainSettings.contains("sampling_interval")
				? int(localDomainSettings["sampling_interval"])
				: 1;
			...
			if (localDomainSettings.contains("randomSeedGlobal"))	_randomSeedGlobal = localDomainSettings["randomSeedGlobal"].extract<const UInt32>();
			if (localDomainSettings.contains("randomSeedTile"))	_randomSeedTile = localDomainSettings["randomSeedTile"].extract<const UInt32>();
			if (localDomainSettings.contains("randomSeedBlock"))	_randomSeedBlock = localDomainSettings["randomSeedBlock"].extract<const UInt32>();
			if (localDomainSettings.contains("randomSeedCell"))	_randomSeedCell = localDomainSettings["randomSeedCell"].extract<const UInt32>();
			...
			if (spinup->enabled()) {
				...
				for (const auto& spinupModuleConfig : spinup->modules()) {
					...
					_spinupModules[key] = _libraryManager.GetModule(
						spinupModuleConfig->libraryName(),
						spinupModuleConfig->name());
				...
				for (const auto& variable : spinup->variables()) {
					_spinupLandUnitController.addVariable(variable->name(), std::make_shared<Variable>(variable->value(), VariableInfo{ variable->name() }));
				}
				...
				for (const auto& variable : spinup->externalVariables()) {
					...
					spinuptransforms[variableName] = _libraryManager.GetTransform(transformConfig.libraryName(), transformConfig.typeName());
					_spinupLandUnitController.addVariable(variable->name(), std::make_shared<moja::flint::ExternalVariable>(spinuptransforms[variableName], VariableInfo{ variable->name() }));
				}
				...
				for (const auto& variable : spinup->flintDataVariables()) {
					...
					const auto& variableName = variable->name();
					spinupflintdata[variableName] = _libraryManager.GetFlintData(transformConfig.libraryName(), transformConfig.typeName());
					_spinupLandUnitController.addVariable(variable->name(), std::make_shared<moja::flint::FlintDataVariable>(spinupflintdata[variableName], transformConfig.libraryName(), transformConfig.typeName(), VariableInfo{ variable->name() }));
				}
				...
				for (const auto& variable : spinup->flintDataVariables()) {
					...
					const auto& variableName = variable->name();
					spinupflintdata[variableName]->configure(flintDataConfig.settings(), _spinupLandUnitController, _dataRepository);
				}
				...
				for (const auto& variable : spinup->externalVariables()) {
					const auto& transformConfig = variable->transform();
					const auto& variableName = variable->name();
					spinuptransforms[variableName]->configure(transformConfig.settings(), _spinupLandUnitController, _dataRepository);
				}
			...
			MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Run ID = '" << _runId << "'";
			...
			if (_threadedSystem && _isMaster) {
				...
				for (auto t = 0; t < _numThreads; t++) {
					auto task = std::make_shared<InternalThreadBlocks>(
						(t + 100), _runId,
						_blockListMutex, _blockIdxList, _blockIdxListSize, _blockCellIdxList,
						_config,
						_globalStatsDimension, _tileStatsDimension, _blockStatsDimension,
						_internalLIbraryHandles);
		...
		void SpatialTiledLocalDomainController::buildBlockIndexQueue(configuration::LocalDomainIterationType iterationType) {
			...
			case configuration::LocalDomainIterationType::LandscapeTiles: {
				...
				for (auto tile : tiles) {
					...
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":"
						<< "Queuing tile: " << tileIdx.toString() << " "
						<< tileProcessedCount << " of " << tiles.size();
			...
			case configuration::LocalDomainIterationType::BlockIndex: {
				...
				for (auto& block : blocks) {
					...
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":"
						<< "Queuing block: " << blockIdx.toString() << " "
						<< blockProcessedCount << " of " << blocks.size();
		...
		bool SpatialTiledLocalDomainController::runCellSpinUp(std::shared_ptr<StatsUnitRecord>& blockStatsUnit, const datarepository::CellIdx& cell) {
		...
		bool SpatialTiledLocalDomainController::runCell(std::shared_ptr<StatsUnitRecord>& blockStatsUnit, std::shared_ptr<StatsUnitRecord>& blockStatsSpinUpUnit, const datarepository::CellIdx& cell) {
			...
			try {
				...
				if (_randomSeedCell.isNull()) {
					std::uniform_int_distribution<unsigned> distribution(0, std::numeric_limits<unsigned>().max()); // define the range
		...
		//void SpatialTiledLocalDomainController::writeRunSummaryToSQLite(bool insertRunList, std::vector<RunSummaryDataRecord>& runSummaryData) {
		//	auto retry = false;
		//	auto maxRetries = RETRY_ATTEMPTS;
		//	do {
		//		try {
		...
		//			}
		//			if (insertRunList)
		//				session << "CREATE TABLE IF NOT EXISTS run_list (id INTEGER PRIMARY KEY AUTOINCREMENT, run_id VARCHAR(64) NOT NULL, run_desc VARCHAR(255))", now;
		//			session << "CREATE TABLE IF NOT EXISTS run_properties (id INTEGER PRIMARY KEY AUTOINCREMENT, run_id_fk VARCHAR(64), property_name VARCHAR(255), property_info VARCHAR(255), property_value VARCHAR(255))", now;
		//
		//			if (insertRunList) {
		//				auto insertStr = (boost::format("INSERT INTO run_list (run_id, run_desc) VALUES('%1%','%2%')") % _runId % _runDesc).str();
		//				session.begin();
		//				session << insertStr, now;
		//				session.commit();
		//			}
		//
		//			session.begin();
		//			session << "INSERT INTO run_properties (run_id_fk, property_name, property_info, property_value) VALUES(?, ?, ?, ?)", bind(runSummaryData), now;
		//			session.commit();
		//			SQLite::Connector::unregisterConnector();
		//		}
		//		catch (SQLite::DBLockedException&) {
		...
		void SpatialTiledLocalDomainController::run() {
			...
			if (_writeStatsToSQLIte && (!_threadedSystem || (_threadedSystem && _isMaster))) {
				...
				//StatsUnitRecord::sqliteCreateTable(_createStatsTablesSQLIte, localDomainId(), _sqliteDatabaseName, "run_stats_global");
				//StatsUnitRecord::sqliteCreateTable(_createStatsTablesSQLIte, localDomainId(), _sqliteDatabaseName, "run_stats_tile");
				//StatsUnitRecord::sqliteCreateTable(_createStatsTablesSQLIte, localDomainId(), _sqliteDatabaseName, "run_stats_block");
				...
				if (!_threadedSystem && _landUnitController.hasVariable("spatialLocationInfo")) {
					auto _spatiallocationinfo = std::static_pointer_cast<moja::flint::SpatialLocationInfo>(_landUnitController.getVariable("spatialLocationInfo")->value().extract<std::shared_ptr<moja::flint::IFlintData>>());
					runSummaryInfoStr = (boost::format("Single process %1%_%2%_%3%") % _spatiallocationinfo->_tileIdx % _spatiallocationinfo->_blockIdx % _spatiallocationinfo->_cellIdx).str();
				}

				std::vector<RunSummaryDataRecord> runSummaryData;
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_system_start", runSummaryInfoStr, (boost::format("%1%") % moja::put_time(&t1, "%c %Z")).str()));
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_threaded", runSummaryInfoStr, (boost::format("%1%") % (_threadedSystem ? "true" : "false")).str()));
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_num_threads", runSummaryInfoStr, (boost::format("%1%") % _numThreads).str()));
			...
			if (_writeStatsToSQLIte && (_threadedSystem && !_isMaster)) {
				...
				std::vector<RunSummaryDataRecord> runSummaryData;
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_thread_system_start", runSummaryInfoStr, (boost::format("%1%") % moja::put_time(&t1, "%c %Z")).str()));
			...
			if (_threadedSystem && _isMaster) {
				...
				try {
					_spatiallocationinfo = std::static_pointer_cast<flint::SpatialLocationInfo>(
						_landUnitController.getVariable("spatialLocationInfo")->value()
					...
					while (_blockIdxList.size() > 0) {
						...
						auto blockStatsRecord = std::make_shared<StatsUnitRecord>(_runId, 0ns, 0ns, 0ns, 0ns, 0, 0, 0, 0, blockIdx.tileIdx, blockIdx.blockIdx, StatsDurationType::Milliseconds);
						auto blockStatsSpinUpRecord = std::make_shared<StatsUnitRecord>(_runId, 0ns, 0ns, 0ns, 0ns, 0, 0, 0, 0, blockIdx.tileIdx, blockIdx.blockIdx, StatsDurationType::Milliseconds);
						...
						_notificationCenter.postNotification(signals::LocalDomainProcessingUnitInit); // Processing unit is a Set of blocks
						if (_runSpinUp) {
							_spinupNotificationCenter.postNotification(signals::LocalDomainProcessingUnitInit); // Processing unit is a Set of blocks
						}
						...
						if (_blockCellIdxList.empty()) {
							for (const auto& cell : _provider->cells(blockIdx)) {
								...
								MOJA_LOG_TRACE << "Processing cell: " << _spatiallocationinfo->_tileIdx << ", " << _spatiallocationinfo->_blockIdx << ", " << _spatiallocationinfo->_cellIdx;
						...
						blockStatsRecord->mojaLog("Block", localDomainId(), nullptr, &blockIdx, false, true, _blockIdxListPosition, _blockIdxListSize);

						if (_runSpinUp) {
							_spinupNotificationCenter.postNotification(signals::LocalDomainProcessingUnitShutdown);	// End Processing unit
						}
					...
					_spatiallocationinfo->_errorCode = 1;
					MOJA_LOG_FATAL
						<< "Exception caught at LocalDomain level, exiting..."
						<< "Module: " << _spatiallocationinfo->_module << ", "
						<< "Location: " << _spatiallocationinfo->_tileIdx << "," << _spatiallocationinfo->_blockIdx << "," << _spatiallocationinfo->_cellIdx << ", "
						<< "msg:" << e.displayText();
					exit(1);
				}
				catch (const boost::exception& e) {
					_spatiallocationinfo->_errorCode = 1;
					MOJA_LOG_FATAL
						<< "boost::exception caught at LocalDomain level, exiting..."
						<< "Module: " << _spatiallocationinfo->_module << ", "
						<< "Location: " << _spatiallocationinfo->_tileIdx << "," << _spatiallocationinfo->_blockIdx << "," << _spatiallocationinfo->_cellIdx << ", "
					...
					_spatiallocationinfo->_errorCode = 1;
					MOJA_LOG_FATAL
						<< "Poco::FileException caught at LocalDomain level, exiting..."
						<< "Module: " << _spatiallocationinfo->_module << ", "
						<< "Location: " << _spatiallocationinfo->_tileIdx << "," << _spatiallocationinfo->_blockIdx << "," << _spatiallocationinfo->_cellIdx << ", "
						<< "msg:" << e.displayText();
					;
				...
				//} 
					...
					_spatiallocationinfo->_errorCode = 1;
					MOJA_LOG_FATAL
						<< "Poco::Exception caught at LocalDomain level, exiting..."
						<< "Module: " << _spatiallocationinfo->_module << ", "
						<< "Location: " << _spatiallocationinfo->_tileIdx << "," << _spatiallocationinfo->_blockIdx << "," << _spatiallocationinfo->_cellIdx << ", "
						<< "msg: " << e.displayText();
					exit(1);
					...
					_spatiallocationinfo->_errorCode = 1;
					MOJA_LOG_FATAL
						<< "std::exception caught at LocalDomain level, exiting..."
						<< "Module: " << _spatiallocationinfo->_module << ", "
						<< "Location: " << _spatiallocationinfo->_tileIdx << "," << _spatiallocationinfo->_blockIdx << "," << _spatiallocationinfo->_cellIdx << ", "
						<< "msg:" << e.what();
					exit(1);
			...
			if (_doLogging) {
				if (!_threadedSystem || (_threadedSystem && _isMaster)) {
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Summary of processing by Tile";
					...
					//	StatsUnitRecord tileStatsRecord(i.get<1>(), seconds(i.get<2>()), seconds(i.get<3>()), seconds(i.get<4>()), seconds(i.get<5>()), i.get<6>(), i.get<7>(), i.get<8>(), i.get<9>(), i.get<10>(), i.get<11>(), StatsDurationType::Seconds);
					//	datarepository::TileIdx tileIdxObject(i.get<10>(), _provider->indexer());
					//	tileStatsRecord.mojaLog("Tile", localDomainId(), &tileIdxObject, nullptr, true);
					//}

					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Summary of processing for full run";

					//for (auto& i : _globalStatsDimension->getPersistableCollection()) {
					//	StatsUnitRecord tileStatsRecord(i.get<1>(), seconds(i.get<2>()), seconds(i.get<3>()), seconds(i.get<4>()), seconds(i.get<5>()), i.get<6>(), i.get<7>(), i.get<8>(), i.get<9>(), i.get<10>(), i.get<11>(), StatsDurationType::Seconds);
					//	totalUnits += tileStatsRecord._unitsTotal;
					//	totalUnitsProcessed += tileStatsRecord._unitsProcessed;
					//	tileStatsRecord.mojaLog("Global", localDomainId(), nullptr, nullptr, true);
					//}
					...
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Start Time           : " << moja::put_time(&t1, "%c %Z");
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Finish Time          : " << moja::put_time(&t2, "%c %Z");
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Total Time (seconds) : " << duration_cast<seconds>(runTime).count();
				}
				...
				if (_threadedSystem && _isMaster) {
					...
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Effective All LU/millisecond:       " << std::setfill(' ') << std::setw(8) << luPerMillisecond;
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Effective All LU/second:            " << std::setfill(' ') << std::setw(8) << luPerSecond;
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Effective Processed LU/millisecond: " << std::setfill(' ') << std::setw(8) << luProcessedPerMillisecond;
					MOJA_LOG_INFO << std::setfill(' ') << std::setw(3) << localDomainId() << ":" << "Effective Processed LU/second:      " << std::setfill(' ') << std::setw(8) << luProcessedPerSecond;
				}
			}
			...
			if (_writeStatsToSQLIte && (!_threadedSystem || (_threadedSystem && _isMaster))) {
				...
				//StatsUnitRecord::sqliteWrite(_blockStatsDimension,	localDomainId(), _sqliteDatabaseName, "run_stats_block");

				std::vector<RunSummaryDataRecord> runSummaryData;
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_system_finish", runSummaryInfoStr, (boost::format("%1%") % moja::put_time(&t2, "%c %Z")).str()));
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_time_seconds", runSummaryInfoStr, (boost::format("%1%") % durS).str()));
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_time_milliseconds", runSummaryInfoStr, (boost::format("%1%") % durMS).str()));
				...
				if (_threadedSystem && _isMaster) {
					...
					runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_all_effective_lu_per_seconds", runSummaryInfoStr, (boost::format("%1%") % luPerMillisecond).str()));
					runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_all_effective_lu_per_milliseconds", runSummaryInfoStr, (boost::format("%1%") % luPerSecond).str()));
					runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_processed_effective_lu_per_seconds", runSummaryInfoStr, (boost::format("%1%") % luProcessedPerMillisecond).str()));
					runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_processed_effective_lu_per_milliseconds", runSummaryInfoStr, (boost::format("%1%") % luProcessedPerSecond).str()));
				}
				//writeRunSummaryToSQLite(false, runSummaryData);
			}
			if (_writeStatsToSQLIte && (_threadedSystem && !_isMaster)) {
				...
				std::vector<RunSummaryDataRecord> runSummaryData;
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_thread_%1%_system_finish", runSummaryInfoStr, (boost::format("%1%") % moja::put_time(&t2, "%c %Z")).str()));
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_thread_%1%_time_seconds", runSummaryInfoStr, (boost::format("%1%") % durS).str()));
				runSummaryData.push_back(RunSummaryDataRecord(_runId, "run_thread_%1%_time_milliseconds", runSummaryInfoStr, (boost::format("%1%") % durMS).str()));
