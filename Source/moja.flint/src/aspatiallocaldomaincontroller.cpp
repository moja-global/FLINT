    ...
    if (spinup->enabled()) {
        ...
        for (const auto& spinupModuleConfig : spinup->modules()) {
            ...
            _spinupModules[key] = _libraryManager.GetModule(
                spinupModuleConfig->libraryName(), spinupModuleConfig->name());
        ...
        for (const auto& variable : spinup->variables()) {
            _spinupLandUnitController.addVariable(
                variable->name(),
                std::make_shared<Variable>(variable->value(),
        ...
        for (const auto& variable : spinup->externalVariables()) {
            ...
            spinuptransforms[variableName] = _libraryManager.GetTransform(
                transformConfig.libraryName(), transformConfig.typeName());

            _spinupLandUnitController.addVariable(
                variable->name(),
                std::make_shared<moja::flint::ExternalVariable>(
                    spinuptransforms[variableName], VariableInfo{ variable->name() }));
        }
        ...
        for (const auto& variable : spinup->externalVariables()) {
            const auto& transformConfig = variable->transform();
            const auto& variableName = variable->name();
            spinuptransforms[variableName]->configure(
                transformConfig.settings(), _spinupLandUnitController, _dataRepository);
        }
...
void AspatialLocalDomainController::run() {
    ...
    for (auto lu : *_landscape) {
        count++;
        MOJA_LOG_INFO << std::setfill(' ') << std::setw(10) << count << " of "
