...
std::shared_ptr<IOperation> OperationManagerSimple::createProportionalOperation(IModule& module, DynamicVar& dataPackage) {
	return std::make_shared<ProportionalOperationSimple>(this, _poolValues, _timing.fractionOfStep(), &module.metaData(), dataPackage);
}
	...
	if (_useKahan) { // used for https://en.wikipedia.org/wiki/Kahan_summation_algorithm
		...
		for (auto& operationResult : _operationResultsPending) {
			...
			for (auto& flux : p->_fluxes) {
                ...
                if (!_allowNegativeTransfers && FloatCmp::lessThan(val, 0.0)) {
                    ...
                    BOOST_THROW_EXCEPTION(SimulationError()
                        << Details((boost::format("Negative transfer by %1%: %2% %3% %4% to %5%")
                            ...
                            % getPool(srcIx)->name() % getPool(dstIx)->name()).str())
                        << LibraryName("moja.flint")
                        << ModuleName("OperationManagerSimple")
                        << ErrorCode(0));
				}
                ...
                //MOJA_LOG_DEBUG << "Simple applyOperations - pool src pre: " << std::setfill(' ') << std::setw(30) << std::setprecision(15) << _poolValues[srcIx] << ", pool dst pre: " << std::setfill(' ') << std::setw(30) << std::setprecision(15) << _poolValues[dstIx] << ", src: " << std::setfill(' ') << std::setw(3) << srcIx << ", snk: " << std::setfill(' ') << std::setw(3) << dstIx << ", value:" << val << ", ";
				_poolValues[srcIx] -= val;
				_poolValues[dstIx] += val;
				//MOJA_LOG_DEBUG << "Simple applyOperations - pool src pst: " << std::setfill(' ') << std::setw(30) << std::setprecision(15) << _poolValues[srcIx] << ", pool dst pst: " << std::setfill(' ') << std::setw(30) << std::setprecision(15) << _poolValues[dstIx];
			}
		}
	}
...
void OperationManagerSimple::clearLastAppliedOperationResults() {
	_operationResultsLastApplied.clear();
}
...
PoolCollection OperationManagerSimple::poolCollection() {
...
const IPool* OperationManagerSimple::addPool(const std::string& name, const std::string& description, const std::string& units, double scale, int order, const std::shared_ptr<ITransform> transform) {
...
const IPool* OperationManagerSimple::addPool(const std::string& name, const std::string& description, const std::string& units, double scale, int order, double initValue) {
    if (initValue < 0.0) {
...
const IPool* OperationManagerSimple::addPool(const std::string& name, const std::string& description, const std::string& units, double scale, int order, TInitValue initValue) {
    if (name.length() == 0 || all(name, boost::algorithm::is_space())) {
        throw std::invalid_argument("name cannot be empty");
    }

    if (_poolValues.size() == _poolValues.capacity()) {
        throw ApplicationException("maximum pool definitions exceeded. Only 255 pools allowed"); // to protect the references held by PoolSimple wrappers
    }

    _poolValues.push_back(0.0);
    auto pool = std::make_shared<TPool>(_poolValues, name, description, units, scale, order, static_cast<int>(_poolValues.size() - 1), initValue);
...
bool OperationManagerSimple::hasOperationResultsLastApplied() const {
