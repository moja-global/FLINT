...
LocalDomainControllerBase::LocalDomainControllerBase() : _localDomainId(0),
_config(nullptr), _landUnitController(), _dataRepository(), _libraryManager() {
	_dataRepository.setProviderRegistry(_libraryManager.getProviderRegistry());
}

// --------------------------------------------------------------------------------------------

LocalDomainControllerBase::LocalDomainControllerBase(std::shared_ptr<FlintLibraryHandles> libraryHandles) : _localDomainId(0),
	_config(nullptr), _landUnitController(), _dataRepository(),
	_libraryManager(libraryHandles) {
	_dataRepository.setProviderRegistry(_libraryManager.getProviderRegistry());
}
...
void LocalDomainControllerBase::configure(const configuration::Configuration& config) {
	...
	for (auto lib : config.libraries()) {
		...
		case configuration::LibraryType::External: {
			if (lib->hasPath())
			{
				MOJA_LOG_DEBUG << "details (" << lib->name() << "): path - " << lib->path() << ", filename -" << lib->library();
	...
	for (const auto& module : config.modules()) {
		...
		if (_moduleMap.find(key) != _moduleMap.end()) {
			//duplicate Key found
			throw DuplicateModuleDefinedException() << Details("Error duplicate module defined") << LibraryName(module->libraryName()) << SequencerName(module->name());
		}
		...
		if (module->isProxy()) {
			...
			for (auto& item : proxyModules) {
				...
				auto moduleSettings = proxyModule.contains( "settings") ?   proxyModule["settings"].extract<const DynamicObject>() : DynamicObject();
	...
	if (_sequencer == nullptr) {
		throw SequencerNotFoundException() << Details("Error finding sequencer") << LibraryName(config.localDomain()->sequencerLibrary()) << SequencerName(config.localDomain()->sequencer());
	}
	_sequencer->configure(timing);

	// Configure Pools
	for (const auto pool : config.pools()) {
		_landUnitController.operationManager()->addPool(pool->name(), pool->description(), pool->units(), pool->scale(), pool->order(), pool->initValue());
	}
	...
	for (const auto variable : config.externalVariables()) {
		...
		_landUnitController.addVariable(
			variable->name(),
			std::make_shared<ExternalVariable>(
				transforms[key], VariableInfo{ variable->name() }));
	}
    ...
    for (const auto pool : config.externalPools()) {
        ...
        _landUnitController.operationManager()->addPool(poolName, pool->description(), pool->units(), pool->scale(), pool->order(), transforms[key]);
    }
	...
	_landUnitController.initialiseData(false);	/// TODO: check if this is required here. initialiseData is also called in SpatialTiledLocalDomainController::runCell
}
...
void LocalDomainControllerBase::startup() {
	_notificationCenter.postNotification(moja::signals::LocalDomainInit);
}

// --------------------------------------------------------------------------------------------

void LocalDomainControllerBase::shutdown() {
