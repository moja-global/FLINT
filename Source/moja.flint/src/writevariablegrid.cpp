...
void WriteVariableGrid::configure(const DynamicObject& config) {
	...
	for (auto& item : items) {
		...
		if (enabled) {
			...
			if      (variableDataType == "UInt8" )	{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<UInt8 >>(_fileHandlingMutex,  1)); }
			else if (variableDataType == "UInt16")	{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<UInt16>>(_fileHandlingMutex, 12)); }
			else if (variableDataType == "Int16" )	{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<Int16 >>(_fileHandlingMutex,  2)); }
			else if (variableDataType == "Int32" )	{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<Int32 >>(_fileHandlingMutex,  3)); }
			else if (variableDataType == "UInt32")	{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<UInt32>>(_fileHandlingMutex, 13)); }
			else if (variableDataType == "Int64" )	{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<Int64 >>(_fileHandlingMutex, 14)); }
			else if (variableDataType == "UInt64")	{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<UInt64>>(_fileHandlingMutex, 15)); }
			else if (variableDataType == "float" )	{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<float >>(_fileHandlingMutex,  4)); }
			else if (variableDataType == "double")	{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<double>>(_fileHandlingMutex,  5)); }
			else									{ _dataVecT.emplace_back(std::make_unique<DataSettingsT<int   >>(_fileHandlingMutex,  0)); }
...
void WriteVariableGrid::subscribe(NotificationCenter& notificationCenter) {
	...
	notificationCenter.subscribe(signals::LocalDomainProcessingUnitInit			, &WriteVariableGrid::onLocalDomainProcessingUnitInit		, *this);
    notificationCenter.subscribe(signals::OutputStep							, &WriteVariableGrid::onOutputStep							, *this);
    notificationCenter.subscribe(signals::LocalDomainProcessingUnitShutdown		, &WriteVariableGrid::onLocalDomainProcessingUnitShutdown	, *this);
...
void WriteVariableGrid::onLocalDomainInit() {
	_spatialLocationInfo = std::static_pointer_cast<SpatialLocationInfo>(
		...
		.extract<std::shared_ptr<IFlintData>>());
...
template <typename T>
void WriteVariableGrid::DataSettingsT<T>::configure(std::string& globalOutputPath, bool useIndexesForFolderName, bool forceVariableFolderName, bool applyAreaAdjustment, const DynamicObject& config) {
...
void WriteVariableGrid::DataSettingsT<T>::addFlux(const DynamicVar& fluxConfig) {
    ...
    if (fluxGroup.contains("flux_source")) {
        ...
        fluxSource = boost::iequals(src, "disturbance") ? FluxSource::DISTURBANCE
            : boost::iequals(src, "annual_process") ? FluxSource::ANNUAL_PROCESS
            : fluxSource;
    }
...
template <typename T>
void WriteVariableGrid::DataSettingsT<T>::initializeData(std::shared_ptr<const SpatialLocationInfo> spatialLocationInfo, std::vector<T>& data) {
...
//	for (const auto& timestepData : _data) {
	...
	for (auto it = _data.begin(); it != _data.end(); ++it) {
		...
		auto filenameGrd = (boost::format("%1%%2%%3%_%4%_%5%.grd")
			% tileFolder.path()
			% Poco::Path::separator()
			% _name
			% folderLocStr
			% timestepData.first).str();
		auto filenameHdr = (boost::format("%1%%2%%3%_%4%_%5%.hdr")
			% tileFolder.path()
			% Poco::Path::separator()
			% _name
			% folderLocStr
        ...
        out << "data ignore value = " << boost::io::group(std::setfill('0'), std::setprecision(std::numeric_limits<T>::max_digits10), _nodataValue) << std::endl;
		out << "map info = { Geographic Lat / Lon, 1, 1, "
			<< boost::io::group(std::setfill('0'), std::setprecision(8), spatialLocationInfo->_blockLatLon.lon)
			<< ", "
			<< boost::io::group(std::setfill('0'), std::setprecision(8), spatialLocationInfo->_blockLatLon.lat)	// adjust to bottom left corner
			<< ", "
			...
			<< ", WGS-84 }" << std::endl;
		out << "coordinate system string = { GEOGCS[\"GCS_WGS_1984\",DATUM[\"D_WGS_1984\",SPHEROID[\"WGS_1984\",6378137,298.257223563]],PRIMEM[\"Greenwich\",0],UNIT[\"Degree\",0.017453292519943295]] }" << std::endl;
...
template <typename T>
T WriteVariableGrid::DataSettingsT<T>::applyValueAdjustment(std::shared_ptr<const SpatialLocationInfo> spatialLocationInfo, int timestep, const T val) {
...
template <typename T>
void WriteVariableGrid::DataSettingsT<T>::setLUValue(std::shared_ptr<const SpatialLocationInfo> spatialLocationInfo, int timestep) {
...
template <typename T>
void WriteVariableGrid::DataSettingsT<T>::setLUVariableValue(std::shared_ptr<const SpatialLocationInfo> spatialLocationInfo, int timestep) {
    if (_propertyName != "") {
        ...
        if (!_isArray) {
            ...
            _data[timestep][spatialLocationInfo->_cellIdx] = applyValueAdjustment(spatialLocationInfo, timestep, variablePropertyValue.template convert<T>());
        }
    }
    else {
...
template <typename T>
void WriteVariableGrid::DataSettingsT<T>::setLUPoolValue(std::shared_ptr<const SpatialLocationInfo> spatialLocationInfo, int timestep) {
...
template <typename T>
void WriteVariableGrid::DataSettingsT<T>::setLUFluxValue(std::shared_ptr<const SpatialLocationInfo> spatialLocationInfo, int timestep) {
    double sum = 0.0;
...
template <typename T>
void WriteVariableGrid::DataSettingsT<T>::initData(std::shared_ptr<const SpatialLocationInfo> spatialLocationInfo, int timestep) {
