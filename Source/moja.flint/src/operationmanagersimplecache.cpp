...
OperationManagerSimpleCache::OperationManagerSimpleCache(Timing& timing, const DynamicObject& config) : _timing(timing) {
	...
	for (int i = 0; i < OPERATION_MANAGER_CACHE_SIZE; i++) {
		_stockCache.push_back(std::make_shared<StockOperationSimpleCache>(this, _poolValues, _timing.fractionOfStep(), nullptr));
	}
	_stockCachePosition = _stockCache.begin();
	_proportionalCache.reserve(OPERATION_MANAGER_CACHE_SIZE);
	for (int i = 0; i < OPERATION_MANAGER_CACHE_SIZE; i++) {
		_proportionalCache.push_back(std::make_shared<ProportionalOperationSimpleCache>(this, _poolValues, _timing.fractionOfStep(), nullptr));
	}
	_proportionalCachePosition = _proportionalCache.begin();

	for (int i = 0; i < OPERATION_MANAGER_CACHE_SIZE; i++) {
		_operationResultCache.push_back(std::make_shared<OperationResultSimpleCache>(OperationTransferType::Proportional, nullptr));
	}
...
std::shared_ptr<IOperation> OperationManagerSimpleCache::createStockOperation(IModule& module) {
	if (_stockCachePosition == _stockCache.end())
		_stockCachePosition = _stockCache.begin();
...
std::shared_ptr<IOperation> OperationManagerSimpleCache::createProportionalOperation(IModule& module) {
	if (_proportionalCachePosition == _proportionalCache.end())
		_proportionalCachePosition = _proportionalCache.begin();
...
std::shared_ptr<IOperation> OperationManagerSimpleCache::createProportionalOperation(IModule& module, DynamicVar& dataPackage) {
	if (_proportionalCachePosition == _proportionalCache.end())
		_proportionalCachePosition = _proportionalCache.begin();
	...
	if (_useKahan) { // used for https://en.wikipedia.org/wiki/Kahan_summation_algorithm
		...
		for (auto& operationResult : _operationResultsPending) {
			...
			for (auto& flux : p->_fluxes) {
				...
				auto val = flux.value();
				//MOJA_LOG_DEBUG << "SimpleCache applyOperations - pool src pre: " << std::setfill(' ') << std::setw(30) << std::setprecision(15) << _poolValues[srcIx] << ", pool dst pre: " << std::setfill(' ') << std::setw(30) << std::setprecision(15) << _poolValues[dstIx] << ", src: " << std::setfill(' ') << std::setw(3) << srcIx << ", snk: " << std::setfill(' ') << std::setw(3) << dstIx << ", value:" << val << ", ";
				_poolValues[srcIx] -= val;
				_poolValues[dstIx] += val;
				//MOJA_LOG_DEBUG << "SimpleCache applyOperations - pool src pst: " << std::setfill(' ') << std::setw(30) << std::setprecision(15) << _poolValues[srcIx] << ", pool dst pst: " << std::setfill(' ') << std::setw(30) << std::setprecision(15) << _poolValues[dstIx];
			}
		}
	}
...
void OperationManagerSimpleCache::clearLastAppliedOperationResults() {
	_operationResultsLastApplied.clear();
}
...
const IPool* OperationManagerSimpleCache::addPool(const std::string& name, const std::string& description, const std::string& units, double scale, int order, double initValue) {
	...
	auto pool = std::make_shared<PoolSimpleCache>(_poolValues, name, description, units, scale, order, static_cast<int>(_poolValues.size() - 1), initValue);
