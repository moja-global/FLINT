...
LibraryManager::LibraryManager() {
	...
	const auto internalHandles = std::make_shared<FlintLibraryHandles>(
		...
		getDataRepositoryProviderRegistrations);
...
void LibraryManager::AddLibrary(LibraryType libraryType,
	const std::string& inLibraryName,
	...
	default: {
		throw LibraryLoadException("Unknown library type defined", inLibraryName);
	}
	}
...
std::map<std::string, std::string> LibraryManager::FindLibraryPathsInDirectory(
	const std::string& directoryName, const std::string& NamePattern, bool useSuffix) const {
	...
	while (dirIterator != end) {
        ...
        bool suffixMatches = file.size() > suffix.size()
            && boost::iequals(file.substr(file.size() - suffix.size()), suffix);
...
ProviderInterfacePtr LibraryManager::GetProvider(const std::string& libraryName, const std::string& providerName, const DynamicObject& settings) {
	try {
...
void LibraryManager::RegisterModules(std::shared_ptr<LibraryInfoBase> libraryInfo, const std::string& libraryName) {
	...
	for (auto i = 0; i < moduleCount; i++) {
		...
		auto fp = [registration]() {
			return ModuleInterfacePtr(registration.initializer());
		};
		_moduleRegistry.insert(std::make_pair(
			std::make_pair(libraryName, registration.moduleName),
			fp));
	}
}

void LibraryManager::RegisterTransforms(std::shared_ptr<LibraryInfoBase> libraryInfo,
	const std::string& libraryName) {
	
	if (libraryInfo->libraryHandles->getTransformRegistrations == nullptr)
		return;
	...
	for (auto i = 0; i < transformCount; i++) {
		...
		auto fp = [registration]() {
			return TransformInterfacePtr(registration.initializer());
		};
		_transformRegistry.insert(std::make_pair(
			std::make_pair(libraryName, registration.transformName),
			fp));
	}
}

void LibraryManager::RegisterFlintData(std::shared_ptr<LibraryInfoBase> libraryInfo,
	const std::string& libraryName) {

	if (libraryInfo->libraryHandles->getFlintDataRegistrations == nullptr)
		return;
	...
	for (auto i = 0; i < flintDataCount; i++) {
		...
		auto fp = [registration]() {
			return FlintDataInterfacePtr(registration.initializer());
		};
		_flintDataRegistry.insert(std::make_pair(
			std::make_pair(libraryName, registration.flintDataName),
			fp));
	}
}

void LibraryManager::RegisterFlintDataFactory(std::shared_ptr<LibraryInfoBase> libraryInfo, const std::string& libraryName) {
	if (libraryInfo->libraryHandles->getFlintDataFactoryRegistrations == nullptr)
		return;
	...
	for (auto i = 0; i < flintDataFactoryCount; i++) {
		auto registration = registrations[i];
		_flintDataFactories[std::make_tuple(libraryName, registration.libraryName, registration.flintDataName)] = libraryInfo;
		...
		_flintDataFactoryRegistry.insert(std::make_pair(std::make_pair(registration.libraryName, registration.flintDataName), fp));
	}
}

void LibraryManager::RegisterProviders(std::shared_ptr<LibraryInfoBase> libraryInfo, const std::string& libraryName) {
	if (libraryInfo->libraryHandles->getDataRepositoryProviderRegistrations == nullptr)
		return;
	...
	if (libraryInfo->libraryHandles->getModuleRegistrations == nullptr) {
		if (libraryInfo->GetLibraryType() != LibraryType::Internal) {
			throw LibraryLoadException(
				"Attempt to load library already loaded as different type",
...
bool LibraryManager::LoadManagedLibrary() {
	...
	if (libraryInfo->libraryHandles->getModuleRegistrations == nullptr) {
		...
		auto managedLibraryInfo = std::static_pointer_cast<LibraryInfoManaged>(
			_libraries[libraryName]);
		...
		managedLibraryInfo->libraryHandles->getModuleRegistrations					= managedLibraryInfo->handle->hasSymbol("getModuleRegistrations")					? reinterpret_cast<GetModuleRegistrationsFunctionPtr>					(managedLibraryInfo->handle->getSymbol("getModuleRegistrations")				) : nullptr;
		managedLibraryInfo->libraryHandles->getTransformRegistrations				= managedLibraryInfo->handle->hasSymbol("getTransformRegistrations")				? reinterpret_cast<GetTransformRegistrationsFunctionPtr>				(managedLibraryInfo->handle->getSymbol("getTransformRegistrations")				) : nullptr;
		managedLibraryInfo->libraryHandles->getFlintDataRegistrations				= managedLibraryInfo->handle->hasSymbol("getFlintDataRegistrations")				? reinterpret_cast<GetFlintDataRegistrationsFunctionPtr>				(managedLibraryInfo->handle->getSymbol("getFlintDataRegistrations")				) : nullptr;
		managedLibraryInfo->libraryHandles->getFlintDataFactoryRegistrations		= managedLibraryInfo->handle->hasSymbol("getFlintDataFactoryRegistrations")			? reinterpret_cast<GetFlintDataFactoryRegistrationsFunctionPtr>			(managedLibraryInfo->handle->getSymbol("getFlintDataFactoryRegistrations")		) : nullptr;
		managedLibraryInfo->libraryHandles->getDataRepositoryProviderRegistrations	= managedLibraryInfo->handle->hasSymbol("getDataRepositoryProviderRegistrations")	? reinterpret_cast<GetDataRepositoryProviderRegistrationsFunctionPtr>	(managedLibraryInfo->handle->getSymbol("getDataRepositoryProviderRegistrations")) : nullptr;
...
bool LibraryManager::LoadExternalLibrary(
	const std::string& libraryName,
	const std::string& path,
	const std::string& fileName) {
	...
	if (libraryInfo->libraryHandles->getModuleRegistrations == nullptr) {
		...
		externalLibraryInfo->libraryHandles->getModuleRegistrations					= externalLibraryInfo->handle->hasSymbol("getModuleRegistrations")					? reinterpret_cast<GetModuleRegistrationsFunctionPtr>					(externalLibraryInfo->handle->getSymbol("getModuleRegistrations")				) : nullptr;
		externalLibraryInfo->libraryHandles->getTransformRegistrations				= externalLibraryInfo->handle->hasSymbol("getTransformRegistrations")				? reinterpret_cast<GetTransformRegistrationsFunctionPtr>				(externalLibraryInfo->handle->getSymbol("getTransformRegistrations")			) : nullptr;
		externalLibraryInfo->libraryHandles->getFlintDataRegistrations				= externalLibraryInfo->handle->hasSymbol("getFlintDataRegistrations")				? reinterpret_cast<GetFlintDataRegistrationsFunctionPtr>				(externalLibraryInfo->handle->getSymbol("getFlintDataRegistrations")			) : nullptr;
		externalLibraryInfo->libraryHandles->getFlintDataFactoryRegistrations		= externalLibraryInfo->handle->hasSymbol("getFlintDataFactoryRegistrations")		? reinterpret_cast<GetFlintDataFactoryRegistrationsFunctionPtr>			(externalLibraryInfo->handle->getSymbol("getFlintDataFactoryRegistrations")		) : nullptr;
		externalLibraryInfo->libraryHandles->getDataRepositoryProviderRegistrations = externalLibraryInfo->handle->hasSymbol("getDataRepositoryProviderRegistrations")	? reinterpret_cast<GetDataRepositoryProviderRegistrationsFunctionPtr>	(externalLibraryInfo->handle->getSymbol("getDataRepositoryProviderRegistrations")) : nullptr;
...
bool LibraryManager::LoadPythonLibrary(const std::string& libraryName) {
    ...
    if (pyLibraryInfo->moduleHandles.empty()) {
        ...
        for (int i = 0; i < len(keys); i++) {
            ...
            _moduleRegistry.insert(std::make_pair(
                std::make_pair(libraryName, moduleName),
                [pyLibraryInfo, pyModule]() {
