...
std::shared_ptr<IOperation> LandUnitDataWrapper::createProportionalOperation(DynamicVar& dataPackage) {
   return _landUnitController == nullptr ? nullptr
...
std::vector<std::shared_ptr<IVariable>> LandUnitDataWrapper::variables() const {
   return (_landUnitController == nullptr ? std::vector<std::shared_ptr<IVariable>>()
                                          : _landUnitController->variables());
}

bool LandUnitDataWrapper::hasVariable(const std::string& name) const {
   return _landUnitController == nullptr ? false : _landUnitController->hasVariable(name);
}

IOperationManager* LandUnitDataWrapper::operationManager() { return _landUnitController->operationManager(); }

const IOperationManager* LandUnitDataWrapper::operationManager() const {
   return _landUnitController->operationManager();
}

ITiming* LandUnitDataWrapper::timing() { return &_landUnitController->timing(); }
