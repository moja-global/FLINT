...
BOOST_AUTO_TEST_SUITE(SQLQueryTransformTests)
	...
	BOOST_AUTO_TEST_CASE(flint_SQLQueryTransform_IssuesExpectedQueriesUsingVarsAndPools)
	{
		...
		std::vector<ItemVar> testVariables = {
			{ "LandUnitId",			247			, "247"			},
			{ "TestVar01",			99			, "99"			},
			{ "TestVar01_Dummy",	101010101	, "101010101"	},
			{ "a",					"test"		, "'test'"		},
			{ "11",					1.1			, "1.1"			},
			{ "&^%",				true		, "true"		},
		};
		...
		for (auto data : testVariables)
		{
			...
			MOCK_EXPECT(mockLandUnitController->getVariable).with(data.name)
				.returns(mockVariables.back().get());
		...
		// Setup Mock Pools
		std::vector<itemPool> testPools = {
			{ "Pool1",			"mock pool", "n/a", 1.0, 0, 1.1111		, "1.1111"		},
		...
		for (auto& data : testPools)
		{
			...
			mockPools[data.name] = std::make_unique<mocks::MockPool>(data.name, data.description, data.units, data.scale, data.order);
		...
		MOCK_EXPECT(mockPoolCollection.getPoolByName).calls(
				[&mockPools](const std::string name)->mocks::MockPool* 
			{
				return mockPools[name].get();
			});

		MOCK_EXPECT(mockOperationManager->getPoolByName).calls(
				[&mockPools](const std::string name)->mocks::MockPool* 
			{
				return mockPools[name].get();
			});
		...
		std::vector<std::pair<std::string, std::string>> testQueryAndExpected = {
			...
			{
				"{var:" + testVariables[0].name + "} {var:" + testVariables[1].name + "} {var:" + testVariables[2].name + "} {var:" + testVariables[3].name + "} {var:" + testVariables[4].name + "} {var:" + testVariables[5].name + "}",
				testVariables[0].valStr + " " + testVariables[1].valStr + " " + testVariables[2].valStr + " " + testVariables[3].valStr + " " + testVariables[4].valStr + " " + testVariables[5].valStr,
			},
			{	
			...
			{ 
				"{pool:Pool1} {pool:Pool222222} {pool:3}",
				"1.1111 2.22222 333.3333"
			},
			{ 
			...
			{
				"{pool:3} {pool:Pool222222} {pool:Pool1}",
				"333.3333 2.22222 1.1111"
			},
			{
				"SELECT * FROM SPECIES WHERE LandUnitId = {var:LandUnitId}",
				"SELECT * FROM SPECIES WHERE LandUnitId = 247" 
			},
			{
			...
			{
				"SELECT * FROM SPECIES WHERE LandUnitId = {pool:Pool1}",
				"SELECT * FROM SPECIES WHERE LandUnitId = 1.1111" 
			},
			{
		...
		MOCK_EXPECT(mockProvider->GetDataSet).calls(
				[&currentExpectedResult](std::string query)->DynamicVar
			{
	...
	BOOST_AUTO_TEST_CASE(flint_SQLQueryTransform_QuotesStringVariablesInSQL)
	{
		...
		MOCK_EXPECT(mockLandUnitController->getVariable).with("AdminBoundary")
			.returns(mockVariable.get());
		...
		// Run Test
		DynamicObject config({
			{ "queryString", testQueryAndExpected.first },
			{ "provider", "test" }
		});
	...
	BOOST_AUTO_TEST_CASE(flint_SQLQueryTransform_AddressSyntaxForMultiValueVariables)
	{
		...
		auto varValue = std::make_unique<DynamicVar>(DynamicObject({
			{ "admin", "test admin boundary" },
			{ "eco", "test eco boundary" }
		}));

		auto mockVariable = std::make_unique<mocks::MockVariable>();
		MOCK_EXPECT(mockVariable.get()->value).returns(*varValue);
		MOCK_EXPECT(mockLandUnitController->getVariable).with(varName)
			.returns(mockVariable.get());
	...
	BOOST_AUTO_TEST_CASE(flint_SQLQueryTransform_ValueReturnsPlainValueIfSingleColumnSingleRowResult)
	{
		...
		auto mockProvider = std::make_shared<mocks::MockProviderRelational>();
		auto result = std::make_shared<DynamicVar>(std::vector<DynamicObject>{
			DynamicObject({ { "a", 1 } })
		});
	...
	BOOST_AUTO_TEST_CASE(flint_SQLQueryTransform_ValueReturnsStructIfMultiColumnSingleRowResult)
	{
		...
		DynamicObject config({
			{ "queryString", "select a, b from foo" },
			{ "provider", "test" }
		});
	...
	BOOST_AUTO_TEST_CASE(flint_SQLQueryTransform_regression_ValueReturnsEmptyDynamicIfNoResultAfterPreviousSuccessfulQuery)
	{
		auto mockLandUnitController = std::make_unique<mocks::MockLandUnitController>();
		auto mockProvider = std::make_shared<mocks::MockProviderRelational>();

		auto firstResult = std::make_shared<DynamicVar>(std::vector<DynamicObject>{
			DynamicObject({ { "a", 1 } })
		});
