...
BOOST_AUTO_TEST_CASE(flint_LookupTransform_ValueReturnsExpectedResultForDynamicToStruct) {
	...
	auto varToValue = std::make_unique<DynamicVar>(DynamicObject({
		{ "a", 1 }, { expectedKey, expectedValue }, { "c", 3 }
	}));
...
BOOST_AUTO_TEST_CASE(flint_LookupTransform_ValueReturnsExpectedResultForStructToMultiStruct) {
	auto varFromValue = std::make_unique<DynamicVar>(DynamicObject({
		{ "key1", "k1v1" }, { "key2", "k2v2" }
	}));
	MOCK_EXPECT(varFrom->value).returns(*varFromValue);
	...
	std::map<std::string, DynamicVar> expectedResult{
		{ "value1", 2 }, { "value2", "b" }
	};
...
BOOST_AUTO_TEST_CASE(flint_LookupTransform_ValueReturnsExpectedResultForStructToMultiStructWithSingleValueColumn) {
	...
	auto varToValue = std::make_unique<DynamicVar>(std::vector<DynamicVar> {
		DynamicObject({ { "key1", "k1v1" }, { "key2", "k2v1" }, { "value1", 1 } }),
...
BOOST_AUTO_TEST_CASE(flint_LookupTransform_ValueReturnsExpectedResultForDynamicToMultiStructWithSingleValueColumn) {
	...
	auto varToValue = std::make_unique<DynamicVar>(std::vector<DynamicVar> {
		DynamicObject({ { "var1", "k1v1" }, { "value1", "test1" } }),
		DynamicObject({ { "var1", "k1v2" }, { "value1", "test2" } }),
...
// struct's keys.
BOOST_AUTO_TEST_CASE(flint_LookupTransform_ValueReturnsExpectedResultForStructToSubsetMultiStructWithSingleValueColumn) {
    auto varFromValue = std::make_unique<DynamicVar>(DynamicObject({
        { "key1", "k1v2" }, { "key2", "k2v3" }
    }));
    MOCK_EXPECT(varFrom->value).returns(*varFromValue);
    ...
    auto varToValue = std::make_unique<DynamicVar>(std::vector<DynamicVar> {
        DynamicObject({ { "key2", "k2v1" }, { "value1", 1 } }),
        DynamicObject({ { "key2", "k2v2" }, { "value1", 2 } }),
