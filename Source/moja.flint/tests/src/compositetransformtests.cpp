...
BOOST_AUTO_TEST_CASE(flint_CompositeTransform_ConfigureThrowsExceptionOnDuplicateVariableNames) {
    ...
    BOOST_CHECK_THROW(transform.configure(config, mockController, mockRepository),
        IncompleteConfigurationException);
}
...
BOOST_AUTO_TEST_CASE(flint_CompositeTransform_LongFormat) {
    ...
    DynamicVar var1Value = std::vector<DynamicObject>{ DynamicObject({ { "a", 1 }, { "b", 2 } }) };
    DynamicVar var2Value = std::vector<DynamicObject>{ DynamicObject({ { "a", 3 }, { "b", 4 } }),
                                                    DynamicObject({ { "a", 5 }, { "b", 6 } }) };
    MOCK_EXPECT(var1->value).returns(var1Value);
    MOCK_EXPECT(var2->value).returns(var2Value);
