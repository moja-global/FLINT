...
public:
	TestNotificationModule(std::vector<std::string>& callSequence) : ModuleBase(), _callCount(0), _sharedCallSequence(callSequence), _localDomainId(-1) {}
...
#define POST_NOT_AND_RECORD(s,v)			{ _notificationCenter.postNotification(s);		v.push_back(#s); }
#define POST_NOT_WITH_ARG_AND_RECORD(s,a,v)	{ _notificationCenter.postNotification(s,a);	v.push_back(#s); }
...
#define POST_NOT_AND_RECORD(s,v)			{ _notificationCenter.postNotificationWithPostNotification(s);		v.push_back(#s); }
#define POST_NOT_WITH_ARG_AND_RECORD(s,a,v)	{ _notificationCenter.postNotificationWithPostNotification(s,a);	v.push_back(#s); }
...
public:
	TestLocalDomainController() : LocalDomainControllerBase(std::make_shared<FlintLibraryHandles>(
		"internal.moja.flint.test", 
		getModuleRegistrations, 
		getTransformRegistrations, 
		getFlintDataRegistrations,
		getFlintDataFactoryRegistrations,
		getDataRepositoryProviderRegistrations)) {}
	~TestLocalDomainController() = default;

	virtual void configure(const flint::configuration::Configuration& config) override { LocalDomainControllerBase::configure(config); }
